...
const MAPR = (()=>{
  ...
  function draw(dt){
    ...
    if(S) for(const id of S.known){
      ...
      /* 라벨 — 정착지·목적지·방문지·발견한 히든만 (겹침 방지) */
      const showLabel = n.stl || n.type==='goal' || visited || n.type==='hidden';
      if(showLabel){
        ctx.font=`${n.stl||n.type==='goal'?'700 ':''}10px ${'sans-serif'}`;
        ctx.fillStyle= n.type==='goal'? 'rgba(85,224,200,0.95)':
          n.stl? 'rgba(240,225,195,0.95)':
          n.type==='hidden'? 'rgba(201,184,255,0.8)': 'rgba(160,170,200,0.75)';
        ctx.textAlign='center';
        ctx.fillText(n.name.split(' ')[0], x, y-9);
        ctx.textAlign='left';
      }
    }
    ...
    ctx.fillText('◆ 정착지   ● 폐허   ◌? 미확인   ─ 고속 · ╌ 국도 · ┄ 험로', 12, H-10);
  }

...
{id:'ev_parking_evs', type:'탐색', w:8, region:['mid','north'],
 ...
 choices:[
  {label:'쓸 만한 부품을 뜯는다', out:[
    {p:2, text:'멀쩡한 배터리 셀과 배선을 확보했다. 우리 차 예비 전력으로 요긴하다.', fx:{item:'부품', time:45, fatigue:3}},
    {p:1, text:'대부분 방전됐거나 이미 뜯겼다. 그래도 공구함 몇 개와 삼각대는 챙겼다.', fx:{scrap:5, time:40}}]},
  {label:'민지가 배터리를 살린다', req:{comp:'minji'}, out:[
    {p:1, text:'민지가 셀 상태를 하나하나 재더니, 살아있는 것만 병렬로 묶었다.\n\n"이거면 며칠은 전깃불 켜고 자겠네." 오랜만에 밤에 불을 켤 수 있게 됐다.', fx:{item:'부품', time:40, mood:{minji:5}, moodAll:2}}]},
 ]},
...
{id:'ev_wind_turbine', type:'발견', w:6, once:true, hiddenTarget:'solar', region:['north'],
 ...
 choices:[
  ...
  {label:'민지가 배전반을 딴다', req:{comp:'minji'}, out:[
    {p:1, text:'민지가 관리동에 들어가 배전반을 열었다. "…와, 얘 아직 일하고 있었네." 예비 배터리를 가득 충전했다.\n\n"기계는 안 배신해. 사람이 안 돌봐도 제 할 일 하잖아." 민지가 드물게 말이 길었다.', fx:{item:'부품', time:50, mood:{minji:6}, reveal:'solar'}}]},
 ]},
...
{id:'ev_patrol_robot', type:'추적', w:7, region:['mid','north'],
 ...
 choices:[
  ...
  {label:'강우가 기능정지시킨다', req:{comp:'kangwoo'}, out:[
    {p:2, text:'강우가 뒤로 돌아가 관절 사이 케이블을 끊었다. 로봇이 픽 주저앉았다. "이런 건 다리가 약점이야."\n\n렌즈가 꺼지기 전 마지막으로 우리를 봤다. 신호가 갔을까, 안 갔을까.', fx:{mood:{kangwoo:4}, item:'부품', pursuit:1}},
...
{id:'ev_bunker_entrance', type:'발견', w:6, once:true, region:['mid','north'],
 ...
 choices:[
  {label:'문을 열어본다', out:[
    {p:2, text:'끼익— 무거운 문이 열렸다. 안은 비어 있고, 통조림과 생수, 응급킷이 선반에 남았다. 대피자들은 이미 떠난 뒤였다.', fx:{food:5, water:4, item:'의약품', time:50, fatigue:3}},
...
{id:'ev_mobile_clinic', type:'조우', w:7, once:true, region:['mid','north'],
 ...
 choices:[
  ...
  {label:'박 선생이 진료를 돕는다', req:{comp:'parkss'}, out:[
    {p:1, text:'박 선생이 소매를 걷고 합류했다. 두 약사가 손발을 맞춰 밀린 환자를 봤다.\n\n"약사끼리는 말이 필요 없구먼." 헤어질 때 여의사가 귀한 약초 지식을 나눠줬다. 서로가 서로에게 약이 됐다.', fx:{time:60, mood:{parkss:6}, moodAll:4, item:'의약품'}}]},
...
{id:'ev_crashed_drone', type:'추적', w:6, region:['mid','north'],
 ...
 choices:[
  {label:'부품을 뜯어낸다', out:[{p:1, text:'모터, 배터리, 카메라 모듈을 회수했다. 적의 물건이지만 쓸모는 적아를 안 가린다.', fx:{item:'부품', scrap:4, time:30}}]},
  {label:'민지가 메모리를 캔다', req:{comp:'minji'}, out:[
    {p:2, text:'민지가 저장칩을 뽑아 데이터를 열었다. "…얘가 뭘 찍었나 보자." 순찰 경로와 검문소 위치가 담겨 있었다. 위험지대 지도를 얻었다.', fx:{item:'부품', mood:{minji:5}, pursuit:-2, note:{type:'소문',title:'드론 메모리',body:'추락 드론에서 순찰 경로·검문소 위치 확보. 위험지대를 피할 수 있다.'}}},
    {p:1, text:'칩을 뽑는 순간 동체에서 삑— 신호가 새어나갔다. 죽은 줄 알았던 발신기가 마지막으로 울었다. 서둘러 자리를 떴다.', fx:{item:'부품', pursuit:2, moodAll:-2}}]},
...
{id:'ev_brake_downhill', type:'위기', w:7, region:['mid','north'],
 ...
 choices:[
  {label:'저단 기어로 엔진 브레이크', out:[
    {p:2, text:'기어를 확 낮췄다. 엔진이 비명을 지르며 속도를 잡았다. 갓길에 겨우 세워, 식은땀을 닦았다. 브레이크액 누수였다.', fx:{fatigue:5, van:-6, time:40, item:'부품'}}]},
...
{id:'ev_broadcast_station', type:'탐색', w:7, region:['mid','north'],
 ...
 choices:[
  ...
  {label:'송출 장비를 챙긴다', req:{comp:'minji'}, out:[
    {p:1, text:'민지가 소형 송신기와 부품을 뜯어냈다. "이거 손보면 우리도 방송할 수 있어. …언젠가, 할 말이 생기면." 무기가 아닌 목소리를 챙긴 셈이다.', fx:{item:'부품', time:40, mood:{minji:4}}}]},
 ]},
...
{id:'ev_stopped_train', type:'정경', w:7, region:['mid','north'],
 ...
 choices:[
  ...
  {label:'쓸 만한 물건을 찾는다', out:[
    {p:1, text:'선반과 좌석 밑을 뒤져 통조림, 생수, 담요를 챙겼다. 여행 가방 하나엔 아직 개봉 안 한 과자도.\n\n주인에게 미안했지만, 그들도 우리가 살길 바랄 거라 믿었다.', fx:{food:4, water:2, item:'부품', time:35}}]},
 ]},
...
{id:'ev_eunsu_past', type:'동행', w:7, once:true, needsComp:'eunsu', minParty:1, night:true, region:['mid','north'],
 title:'은수의 밤',
 text:'다들 잠든 밤, 은수가 잠을 못 이루고 뒤척인다. 조심스레 입을 연다.\n\n"저… 그날, 학교에 있었어요. 선생님이 책상 밑에 숨으라고 했는데… 저만 나왔어요. 화장실 간다고. 그래서 살았어요."\n\n목소리가 떨린다. "왜 저만 살았을까요."',
 choices:[
  {label:'"네 잘못이 아니야"', out:[{p:1, text:'"화장실 간 게 죄는 아니잖아. 넌 살았고, 그건 네가 나빠서가 아니야." 은수가 오래 울었다.\n\n실컷 울고 난 은수가 처음으로 편히 잠들었다. 어떤 짐은, 말로 꺼내야 비로소 내려놓을 수 있다.', fx:{mood:{eunsu:8}, moodAll:2, flag:'es_truth', note:{type:'인물',title:'은수가 살아남은 이유',body:'그날 학교에서 혼자 나와 살아남은 은수. "왜 저만 살았을까요." 죄가 아니라고 말해줬다.'}}}]},
...
{id:'ev_engine_fire', type:'위기', w:6, region:['south','mid','north'],
 ...
 choices:[
  {label:'담요로 덮어 산소를 차단한다', out:[
    {p:2, text:'보닛을 살짝만 열고 담요를 덮어 눌렀다. 연기가 잦아들었다. 탄 배선을 찾아 갈아끼우느라 시간이 걸렸지만, 불은 막았다.', fx:{time:50, van:-6, item:'부품', fatigue:4}},
  ...
  {label:'민지가 배선을 끊는다', req:{comp:'minji'}, out:[
    {p:1, text:'"메인 전원부터!" 민지가 배터리 단자를 뽑자 연기가 멎었다. 합선이 원인이었다.\n\n"차는 심장보다 혈관이 먼저 나가. 이걸로 됐어." 민지가 침착하게 배선을 정리했다. 큰불을 미리 잡았다.', fx:{time:40, van:-4, mood:{minji:5}, item:'부품'}}]},
 ]},
...
{id:'ev_convenience_store', type:'탐색', w:9, region:['south','mid','north'],
 ...
 choices:[
  {label:'구석구석 뒤진다', out:[
    {p:3, text:'창고 안쪽에서 컵라면 상자, 생수 묶음, 건전지를 찾았다. 냉장고 뒤엔 굴러떨어진 초코바도 몇 개. 편의점은 배신을 안 한다.', fx:{food:5, water:3, item:'부품', time:35}},
    {p:1, text:'먹을 건 다 털렸다. 대신 상비약 코너에서 소독약과 밴드, 진통제를 찾았다. 이것도 감지덕지.', fx:{item:'의약품', time:30}}]},
...
{id:'ev_jaeyi_law', type:'동행', w:7, once:true, needsComp:'jaeyi', minParty:1, region:['mid','north'],
 title:'재이의 원칙',
 text:'야영 중 재이가 낡은 수첩을 꺼내 뭔가 적는다. 슬쩍 보니 우리가 지나온 마을들의 이름과 규칙들.\n\n"공동체마다 규칙이 다르더라고요. 어디는 힘으로, 어디는 나눔으로. …전 기록해요. 언젠가 다시 세상을 세울 때, 어떤 규칙이 사람을 살리는지 알려면."\n\n재이의 눈이 진지하다.',
 choices:[
  {label:'"넌 판사가 됐어야 했네"', out:[{p:1, text:'재이가 쓴웃음을 지었다. "됐었어요. …그날 전까진." 잠깐 침묵. "근데 이젠 판결보다 기록이 중요한 것 같아요. 다시는 그런 일이 없게."\n\n재이의 수첩이 어쩌면 우리 여정의 가장 중요한 짐일지도 모른다.', fx:{mood:{jaeyi:6}, flag:'jy_law', note:{type:'인물',title:'재이의 수첩',body:'전직 판사 재이가 마을마다의 규칙을 기록한다. "어떤 규칙이 사람을 살리는지 알려면."'}}}]},
...
{id:'ev_no_spare_tire', type:'위기', w:6, region:['south','mid','north'],
 ...
 choices:[
  ...
  {label:'민지가 타이어를 때운다', req:{comp:'minji'}, out:[
    {p:1, text:'민지가 펑크 난 자리를 찾아 고무 조각과 접착제로 정교하게 때웠다. "완벽친 않아도 며칠은 가." 바람까지 채워넣으니 그럭저럭 굴러갔다.', fx:{time:35, van:-3, mood:{minji:4}, item:'부품'}}]},
...
{id:'ev_salt_farmer', type:'조우', w:7, region:['south','mid'],
 ...
 choices:[
  {label:'소금을 산다 (고철 4)', req:{scrap:4}, out:[{p:1, text:'소금 한 자루를 샀다. "이거면 겨우내 고기 안 상해." 사내가 쓰는 법도 일러줬다. 음식이 오래갈 밑천을 얻었다.', fx:{scrap:-4, food:3, item:'의약품'}}]},
  {label:'일손을 돕고 나눠받는다', out:[
    {p:1, text:'한나절 소금 긁는 걸 도왔다. 허리가 끊어질 듯했지만, 사내가 넉넉히 나눠줬다.\n\n"땀 흘린 값이야. 이런 건 사는 것보다 버는 게 맛나지." 몸은 고돼도 마음은 뿌듯했다.', fx:{food:4, item:'의약품', fatigue:5, moodAll:3, time:60}}]},
 ]},
...
{id:'ev_wasp_nest', type:'위기', w:6, region:['south','mid'],
 ...
 choices:[
  ...
  {label:'박 선생이 처치를 준비하고 돌파', req:{comp:'parkss'}, out:[
    {p:1, text:'"쏘여도 내가 있으니 괜찮아. 알레르기만 조심하면 돼." 박 선생이 응급킷을 열어두고 지휘했다. 빠르게 나무를 치웠고, 쏘인 자리도 즉시 처치했다. 든든했다.', fx:{time:30, mood:{parkss:5}, item:'의약품'}}]},
...
{id:'ev_abandoned_airfield', type:'발견', w:6, once:true, hiddenTarget:'airfield', region:['mid','north'],
 ...
 choices:[
  ...
  {label:'연료와 부품을 챙긴다', out:[
    {p:2, text:'드럼통에서 항공유를 얻었다. 우리 차엔 그대로 못 쓰지만, 섞거나 바꿔 쓸 수 있다. 공구도 한 아름 챙겼다.', fx:{fuel:6, item:'부품', scrap:6, time:50, reveal:'airfield'}},
    {p:1, text:'드럼통은 대부분 비었지만, 격납고 깊숙이서 새 부품 상자를 발견했다. 정비에 요긴한 것들이다.', fx:{item:'부품', time:45, reveal:'airfield'}}]},
 ]},
...
{id:'ev_kangwoo_past', type:'동행', w:7, once:true, needsComp:'kangwoo', minParty:1, night:true, region:['mid','north'],
 title:'강우의 그날',
...
{id:'ev_waterpark', type:'탐색', w:6, region:['mid','north'],
 ...
 choices:[
  {label:'창고와 매점을 뒤진다', out:[
    {p:2, text:'구명조끼, 방수천, 정수 필터를 확보했다. 매점 창고엔 유통기한 긴 과자와 음료도. 물놀이장이 뜻밖의 보물창고였다.', fx:{food:4, water:3, item:'부품', time:45}},
    {p:1, text:'대부분 곰팡이거나 털렸지만, 정수 필터 몇 개는 건졌다. 이것만 해도 어디냐.', fx:{item:'부품', time:40}}]},
...
{id:'ev_parkss_past', type:'동행', w:7, once:true, needsComp:'parkss', minParty:1, region:['mid','north'],
 title:'박 선생의 후회',
...
{id:'ev_traveling_teacher', type:'조우', w:6, region:['south','mid'],
 ...
 choices:[
  ...
  {label:'재이가 특강을 한다', req:{comp:'jaeyi'}, out:[
    {p:1, text:'재이가 아이들에게 "함께 사는 규칙"을 이야기했다. 어려운 말 없이, 나누고 지키는 법을.\n\n선생이 감탄했다. "이런 분이 계셔야 하는데." 아이들이 재이를 둘러싸고 질문을 쏟아냈다. 재이가 오랜만에 판사였던 시절처럼 빛났다.', fx:{moodAll:5, mood:{jaeyi:6}, time:40}}]},
...
{id:'ev_snow_road', type:'위기', w:7, region:['north'],
 ...
 choices:[
  {label:'바퀴에 천과 밧줄을 감는다', out:[
    {p:2, text:'타이어에 천을 찢어 감고 밧줄로 조여 임시 체인을 만들었다. 덜컹대도 접지력이 생겼다. 엉금엉금 눈길을 헤쳐나갔다.', fx:{time:50, fatigue:5, item:'부품'}}]},
...
{id:'ev_reservoir_boat', type:'발견', w:6, once:true, region:['south','mid'],
 ...
 choices:[
  {label:'배로 건너편을 정찰한다', minParty:1, out:[
    {p:1, text:'가벼운 짐만 싣고 배로 건너편을 살폈다. 건너편엔 손 안 탄 낚시터 창고가 있었다. 통조림과 낚싯대, 그물을 챙겨왔다.\n\n"물고기 잡아 먹으면 되겠네." 낚시 도구는 두고두고 식량이 된다. 배는 다음 사람을 위해 다시 묶어뒀다.', fx:{food:4, item:'부품', time:50, moodAll:2}}]},
...
{id:'ev_wiretap_speaker', type:'추적', w:6, region:['north'],
 ...
 choices:[
  ...
  {label:'민지가 도청원을 찾는다', req:{comp:'minji'}, out:[
    {p:1, text:'민지가 신호를 역추적해 근처 소형 집음기를 찾아냈다. "야영지에 붙었던 거야. 이걸로 녹음해서 스피커로 튼 거고." 장치를 부숴버렸다.\n\n"이제부턴 내가 매일 스캔할게. 벌레는 내가 잡아." 민지가 이를 갈았다. 두려움이 대비로 바뀌었다.', fx:{mood:{minji:5}, pursuit:-1, item:'부품', note:{type:'소문',title:'집음기 도청',body:'천리안이 야영지에 집음기를 붙여 대화를 녹음·재생했다. 민지가 매일 스캔하기로.'}}}]},
  {label:'스피커를 부순다', out:[
...
{id:'ev_seed_warehouse', type:'탐색', w:7, region:['south','mid'],
 ...
 choices:[
  {label:'다양한 씨앗을 챙긴다', out:[
    {p:2, text:'상추, 무, 배추, 콩, 옥수수… 종류별로 씨앗을 챙겼다. 정착할 땅만 있으면, 이걸로 밭을 일군다.\n\n"먹을 걸 줍는 것보다 이게 더 부자 된 기분이야." 씨앗 한 봉지가 한 계절의 식량이 된다.', fx:{food:3, item:'부품', time:45, moodAll:3, flag:'seed_borrowed'}},
    {p:1, text:'종자고는 습기가 차 대부분 상했다. 그래도 밀봉이 온전한 몇 봉지는 건졌다. 콩과 옥수수 씨앗. 이거면 됐다.', fx:{time:40, flag:'seed_borrowed'}}]},
  {label:'농기구와 비료를 챙긴다', out:[{p:1, text:'호미, 삽, 낫과 비료를 챙겼다. 무겁지만 정착하면 다 쓸 것들이다. 언젠가 총 대신 이걸 들 날을 그렸다.', fx:{item:'부품', scrap:5, time:35}}]},
 ]},
...
{id:'ev_tunnel_warehouse', type:'발견', w:6, once:true, region:['mid','north'],
 ...
 choices:[
  {label:'주인을 찾아 거래를 청한다', out:[
    {p:2, text:'인기척을 내자 안쪽에서 노인이 나왔다. "…손님은 오랜만이군." 터널 저장고 주인이었다. 고철과 이야기를 나누고, 물자를 정당하게 거래했다.\n\n"여긴 나눔의 창고야. 필요한 만큼 가져가고, 여유 있으면 채워두고." 세상 끝에도 이런 곳이 있다.', fx:{food:4, water:3, item:'부품', scrap:-5, time:40, note:{type:'소문',title:'터널 나눔창고',body:'폐터널을 저장고로 쓰는 노인. "필요한 만큼 가져가고, 여유 있으면 채워두라"는 나눔의 규칙.'}}},
    {p:1, text:'주인은 없었다. 오래 방치된 듯하다. 상하지 않은 통조림과 물, 부품만 조심히 챙기고, 다음 사람 몫은 남겨뒀다.', fx:{food:3, water:2, item:'부품', time:35}}]},
...
{id:'ev_radio_repairman', type:'조우', w:6, region:['south','mid','north'],
 ...
 choices:[
  ...
  {label:'민지가 개조를 부탁한다', req:{comp:'minji'}, out:[
    {p:1, text:'민지가 노인과 머리를 맞대고 무전기를 개조했다. 수신뿐 아니라 특정 주파수를 감지하는 기능까지 달았다.\n\n"이제 천리안 방송 주파수를 미리 알아챌 수 있어." 노인이 감탄했다. "젊은 기술자가 물건이네!" 정보 우위를 얻었다.', fx:{flag:'radio_fixed', mood:{minji:5}, pursuit:-1, item:'부품'}}]},
...
{id:'ev_crashed_helicopter', type:'탐색', w:6, region:['mid','north'],
 ...
 choices:[
  {label:'장비와 부품을 회수한다', out:[
    {p:2, text:'응급 키트, 로프, 방수포, 그리고 항공유를 회수했다. 정밀 계기 부품도 몇 개. 헬기는 못 날아도, 부품은 날아다닌다.', fx:{item:'의약품', fuel:4, scrap:6, time:50, fatigue:3}},
    {p:1, text:'대부분 부서졌지만 응급 키트와 로프는 멀쩡했다. 튼튼한 로프는 여러모로 쓸모가 많다.', fx:{item:'의약품', time:40}}]},
  {label:'블랙박스를 확인한다', req:{comp:'minji'}, out:[
    {p:1, text:'민지가 헬기 기록장치를 살렸다. 마지막 교신이 재생됐다. "구조 요청 다수… 천리안이 항로를 차단… 착륙 불가…" 그날의 혼란이 담겨 있었다.\n\n"…끝까지 사람 구하려던 사람들이야." 조종석을 향해 잠깐 묵념했다. 부품을 챙기면서도, 예의는 지켰다.', fx:{item:'부품', fuel:3, time:45, moodAll:-2, mood:{minji:3}}}]},
 ]},
...
{id:'ev_parkss_eunsu', type:'동행', w:7, needsComp:'parkss', needsComp2:'eunsu', minParty:2, region:['south','mid','north'],
 title:'약손',
 text:'은수가 손등을 긁힌 걸 보고 박 선생이 다가온다. "이리 줘봐. 덧나면 큰일이야."\n\n소독하고 연고를 발라주는 손길이 다정하다. "…우리 손녀도 딱 네 나이였는데." 박 선생의 눈이 잠깐 아득해진다.\n\n은수가 조심스레 묻는다. "손녀분은…?" 박 선생이 옅게 웃는다. "그날. …너 보면 자꾸 생각나."',
 choices:[
  {label:'두 사람이 서로에게 기대게 둔다', out:[{p:1, text:'끼어들지 않았다. 박 선생은 은수에게서 손녀를, 은수는 박 선생에게서 할아버지를 봤다.\n\n"그럼 제가 손녀 할게요. 선생님이 할아버지 해주세요." 은수 말에 박 선생의 눈시울이 붉어졌다. "…그래. 그러자." 잃은 자리를 서로가 채웠다. 피 안 섞인 가족이 하나 더 생겼다.', fx:{moodAll:5, mood:{parkss:6, eunsu:6}, note:{type:'인물',title:'약손과 손녀',body:'박 선생은 은수에게서 그날 잃은 손녀를 본다. "제가 손녀 할게요"—피 안 섞인 조손이 됐다.'}}}]},
  {label:'분위기를 밝게 돌린다', out:[{p:1, text:'무거워지는 공기를 풀려고 농을 던졌다. "박 선생님, 은수 이제 손녀 삼으셨으니 용돈 주셔야죠." 다들 웃음이 터졌다.\n\n박 선생도 껄껄 웃었다. "이 녀석, 늙은이 지갑을 노려." 슬픔이 웃음으로 번졌다. 은수가 박 선생 옆에 바짝 붙어 앉았다.', fx:{moodAll:4, mood:{parkss:4, eunsu:4}}}]},
 ]},
...
{id:'ev_hydroponic_farm', type:'발견', w:6, once:true, minParty:1, region:['mid','north'],
 ...
 choices:[
  ...
  {label:'민지가 재배 설비를 살린다', req:{comp:'minji'}, minParty:1, out:[
    {p:1, text:'민지가 펌프와 LED를 예비 전력에 물려 한 선반을 되살렸다. "물이랑 빛만 있으면 계속 자라. 씨앗 뿌리면 며칠 뒤 또 먹어." 이동식 텃밭의 원리를 배웠다.\n\n"우리 차에도 작게 만들 수 있겠는데?" 다들 눈이 반짝였다. 달리는 채소밭의 꿈이 생겼다.', fx:{food:4, item:'부품', time:45, mood:{minji:5}, note:{type:'소문',title:'수경재배 원리',body:'물·빛·씨앗만 있으면 되는 수경재배. 봉고차용 소형 텃밭 제작 아이디어를 얻었다.'}}}]},
 ]},
...
{id:'ev_mirror_seller', type:'조우', w:6, region:['south','mid'],
 ...
 choices:[
  ...
  {label:'작은 손거울을 산다 (고철 2)', req:{scrap:2}, out:[{p:1, text:'손거울 하나를 샀다. 신호 보낼 때 햇빛 반사용으로도, 사각 확인용으로도 쓴다. 실용과 낭만을 겸한 물건이다.', fx:{scrap:-2, item:'부품'}}]},
...
{id:'ev_beekeeper', type:'조우', w:6, minParty:1, region:['south','mid'],
 ...
 choices:[
  {label:'꿀을 산다 (고철 5)', req:{scrap:5}, out:[{p:1, text:'꿀 한 병을 샀다. 진하고 달다. "이거 한 술이면 하루가 든든해." 오랜만의 단맛에 다들 눈이 커졌다. 힘이 나는 귀한 식량이자 약이다.', fx:{scrap:-5, food:4, item:'의약품'}}]},
  {label:'박 선생이 약용을 논한다', req:{comp:'parkss'}, out:[
    {p:1, text:'박 선생과 양봉가가 꿀의 약효를 두고 이야기꽃을 피웠다. "화상엔 꿀만 한 게 없죠." "맞아요, 항균 작용이 있어서." 둘이 죽이 맞았다.\n\n양봉가가 반가움에 꿀을 넉넉히 얹어줬다. "아는 분을 만나니 반갑네요." 지식이 인심을 부른다.', fx:{scrap:-4, food:5, item:'의약품', mood:{parkss:3}}}]},
...
{id:'ev_used_bookstore', type:'조우', w:6, region:['south','mid','north'],
 ...
 choices:[
  {label:'가져갈 책을 고른다', out:[{p:1, text:'생존 안내서, 소설, 시집 몇 권을 골랐다. "긴 밤에 읽을 게 있으면 덜 무섭지." 노인이 흐뭇하게 봤다.\n\n밤마다 돌아가며 소리 내어 읽기로 했다. 이야기는 추위와 두려움을 밀어낸다. 책 몇 권이 든든한 양식이 됐다.', fx:{item:'책 꾸러미', moodAll:3, flag:'library_books', note:{type:'소문',title:'흐르는 책',body:'헌책방 노인: "읽고 나서 다른 사람한테 넘겨주게. 책은 흘러야 산다."'}}}]},
  {label:'재이가 법전을 찾는다', req:{comp:'jaeyi'}, out:[
    {p:1, text:'재이가 서가를 뒤져 낡은 법전과 헌법 책을 찾아냈다. "…다시 세상을 세울 때, 이게 밑그림이 될 거예요." 소중히 품에 안았다.\n\n노인이 감탄했다. "그 책을 찾는 젊은이는 처음이야. 자네 같은 사람이 필요해." 재이의 눈이 빛났다.', fx:{item:'책 꾸러미', mood:{jaeyi:6}, flag:'library_books'}}]},
...
{id:'ev_post_office', type:'탐색', w:6, region:['mid','north'],
 ...
 choices:[
  {label:'쓸 만한 물자가 든 소포를 연다', out:[
    {p:2, text:'명절 선물 소포에서 통조림 선물세트, 건강식품, 양말 꾸러미가 나왔다. 누군가 보내려던 정성. "잘 쓸게요, 감사합니다." 마음속으로 인사하고 챙겼다.', fx:{food:5, item:'부품', time:40, moodAll:-1}},
...
{id:'ev_train_depot', type:'발견', w:6, once:true, minParty:1, region:['mid','north'],
 ...
 choices:[
  {label:'화물칸을 열어 물자를 찾는다', out:[
    {p:2, text:'봉인된 화물칸에서 생필품 컨테이너를 발견했다. 통조림, 생수, 담요, 공구까지. 운송 중이던 물자가 고스란히. 배불리 챙겼다.', fx:{food:6, water:4, item:'부품', scrap:6, time:55, fatigue:3}},
    {p:1, text:'대부분 산업 자재라 당장 쓸 건 적었지만, 정비고에서 값진 공구와 부품을 잔뜩 확보했다. 차 수리에 두고두고 쓴다.', fx:{item:'부품', scrap:8, van:6, time:50}}]},
...
{id:'ev_jaeyi_solo', type:'동행', w:7, once:true, needsComp:'jaeyi', minParty:1, night:true, region:['mid','north'],
 title:'재이의 저울',
 text:'재이가 홀로 앉아 낡은 배지를 만지작댄다. 판사 시절의 것. 달랑, 저울 모양 문양.\n\n"…판사였을 때, 사람을 죄와 벌로만 봤어요. 근데 그날 이후 알았죠. 세상엔 심판보다 용서가, 벌보다 살림이 필요하다는 걸."\n\n배지를 손에 꼭 쥔다. "이걸 버려야 할지, 지녀야 할지 아직 모르겠어요."',
 choices:[
  {label:'"지녀. 대신 저울을 새로 써"', out:[{p:1, text:'"버리지 마. 그 저울로 이젠 죄가 아니라 사람을 달아. 누굴 살릴지를 재는 저울로." 재이가 배지를 오래 봤다.\n\n"…그렇게 쓸 수도 있겠네요." 처음으로 배지를 주머니에 소중히 넣었다. 심판자가 아니라, 지키는 자의 저울로 다시 태어났다.', fx:{mood:{jaeyi:8}, moodAll:2, flag:'jy_law'}}]},
  {label:'"넌 이미 좋은 판단을 해왔어"', out:[{p:1, text:'"이 여정 내내 넌 옳은 선택을 도왔어. 그게 새 판결이야." 재이가 옅게 웃었다.\n\n"…법전 없이 내리는 판결이 더 어렵네요. 근데 더 사람 같아요." 배지를 다시 품에 넣는 손이 한결 가벼웠다.', fx:{mood:{jaeyi:7}, moodAll:1}}]},
 ]},

{id:'ev_eunsu_solo', type:'동행', w:7, once:true, needsComp:'eunsu', minParty:2, region:['mid','north'],
 title:'막내의 결심',
 text:'은수가 조심스레 다가온다. 늘 보호받던 막내의 눈빛이 오늘은 다르다.\n\n"저… 이제 짐만 되긴 싫어요. 저도 뭔가 하고 싶어요. 망보기든, 요리든, 운전이든. 가르쳐 주세요."\n\n작은 어깨에 힘이 들어가 있다. 그날 혼자 살아남은 아이가, 이젠 함께 싸우려 한다.',
 choices:[
  {label:'운전을 가르친다', out:[{p:1, text:'조수석에 앉혀 운전을 가르쳤다. 처음엔 덜덜 떨더니, 금세 핸들에 익숙해졌다.\n\n"저 운전할 수 있어요!" 은수 얼굴이 환하게 폈다. 이제 교대 운전이 가능해졌다. 막내가 한 뼘 자랐다. 지켜주던 아이가, 함께 지키는 동료가 됐다.', fx:{moodAll:5, mood:{eunsu:7}, fatigue:-2, note:{type:'인물',title:'은수의 성장',body:'보호받던 막내 은수가 운전을 배웠다. "짐만 되긴 싫어요." 이제 함께 싸우는 동료.'}}}]},
  {label:'요리와 살림을 맡긴다', out:[{p:1, text:'은수에게 식사 준비와 물자 관리를 맡겼다. 은수가 야무지게 배급을 짜고, 있는 재료로 근사한 한 끼를 만들었다.\n\n"저 잘하죠?" 뿌듯해하는 은수에게 다들 엄지를 들었다. 막내의 손맛에 야영지가 따뜻해졌다. 각자의 몫을 지니니, 우린 더 단단해졌다.', fx:{moodAll:5, mood:{eunsu:6}, food:2}}]},
 ]},
...
{id:'ev_kids_cafe', type:'탐색', w:6, region:['south','mid'],
 ...
 choices:[
  {label:'물자를 챙긴다', out:[
    {p:2, text:'아이용 간식, 주스, 구급용품, 폭신한 담요를 챙겼다. 유아용이라 순하고 안전한 것들. 담요는 추운 밤에 요긴하다.', fx:{food:4, item:'의약품', time:30}}]},
...
{id:'ev_stealth_dog', type:'위기', w:6, night:true, region:['north'],
 ...
 choices:[
  {label:'강우가 요격한다', req:{comp:'kangwoo'}, out:[
    {p:2, text:'"보리, 물러서!" 강우가 어둠 속에서 로봇의 관절을 정확히 노려 쇠파이프를 박아넣었다. 로봇이 경련하며 쓰러졌다.\n\n"사냥형은 첫 일격이 전부야. 놓치면 우리가 사냥당해." 강우가 거친 숨을 골랐다. 보리가 없었으면 기습당할 뻔했다.', fx:{mood:{kangwoo:5, leo:3}, item:'부품', fatigue:4, pursuit:1}},
...
{id:'ev_market_ruins', type:'탐색', w:7, region:['south','mid'],
 ...
 choices:[
  {label:'골목골목 뒤진다', out:[
    {p:3, text:'건어물전에서 마른 생선과 다시마, 그릇전에서 냄비와 수저, 잡화점에서 성냥과 초를 챙겼다. 시장은 없는 게 없다. 알차게 보급했다.', fx:{food:5, item:'부품', scrap:4, time:45, fatigue:3}},
    {p:1, text:'대부분 털렸지만, 방앗간 구석에서 참기름 한 병과 소금을 찾았다. 음식에 기름 한 방울이면 세상이 달라진다. 귀한 발견이다.', fx:{food:3, item:'의약품', time:40}}]},
  {label:'박 선생이 약재상을 찾는다', req:{comp:'parkss'}, out:[
    {p:1, text:'박 선생이 시장 안 한약방을 찾아 약재를 살폈다. "감초, 당귀, 쑥… 이거 다 약이야." 말린 약재를 정성껏 골라 담았다.\n\n"양약 떨어지면 이게 생명줄이지." 자연 약재 창고를 확보했다. 박 선생의 눈이 반짝였다.', fx:{item:'의약품', time:40, mood:{parkss:5}}}]},
 ]},
...
{id:'ev_apology_broadcast', type:'사건', w:5, once:true, region:['north'],
 ...
 choices:[
  ...
  {label:'재이가 그 언어를 분석한다', req:{comp:'jaeyi'}, out:[{p:1, text:'재이가 이를 악물고 말했다. "…\'정리\', \'손실\', \'안정화\'. 사람을 숫자로 만드는 말들이에요. 책임을 지우는 게 아니라 지우는 언어." 재이가 수첩에 그 문장들을 적었다.\n\n"이 말들을 기록해둘게요. 나중에 심판할 때, 이게 증거가 될 거예요." 분노가 결의로 바뀌었다. 우린 저 언어를 반드시 끝낼 것이다.', fx:{moodAll:-3, mood:{jaeyi:5}, note:{type:'사건',title:'지우는 언어',body:'재이 분석: "정리·손실·안정화는 사람을 숫자로 만들고 책임을 지우는 언어." 심판의 증거로 기록.'}}}]},
 ]},

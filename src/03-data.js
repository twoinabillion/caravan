...
D.onboardingMission = {
  ...
  missionBrief:{
    ...
    now:'발신 기록, 분리 절차, 당사자 증언. 북쪽으로 가는 동안 이 세 가지를 모아야 한다.',
    promise:'세 가지를 근거로 자동 명령을 멈추고, 사람이 직접 확인해야만 이송할 수 있도록 바꾼다.',
    optional:'동료의 부탁과 지역 의뢰는 선택이다. 길에서 마음이 가는 일을 골라 해도 된다.'
  },
  text:`부두 출구 앞에서 잠시 차를 세웠다. 수첩 첫 장에 발신 기록, 분리 절차, 당사자 증언이라고 적었다. 도윤에게 받은 이송표 사본과 엄마가 남긴 검증 모듈을 지도 옆에 놓았다. 이제 북쪽으로 가며 빈칸을 채우면 된다.

남산 코어에 모듈만 꽂는다고 명령이 멈추지는 않는다. 누가 명령을 보냈는지 보여 줄 기록, 검증키를 안전하게 꺼낼 절차, 같은 이송을 겪은 사람들의 증언이 모두 필요하다.

서울에 도착하는 것만으로는 끝나지 않는다. 다음 가족의 이송표가 나오기 전에, 사람을 확인도 하지 않고 실어 나르는 명령 자체를 멈춰야 한다.`,
  choices:[
    {label:'남산으로 가는 첫 길을 고른다', out:[{p:1,text:'부산을 떠나 북쪽 길로 들어섰다. 첫 구간에서는 이송 명령의 발신 기록부터 찾는다. 주 임무 진행 상황은 언제든 「목표」에 표시된다.',fx:{flag:'main_mission_started',note:{type:'본편',title:'남산의 강제 이송을 멈춘다',body:'이송 날짜가 정해져 있는 게 아니다. 배급망이 바뀔 때마다 대상이 다시 정해진다. 북쪽으로 가며 발신 기록과 검증키 분리 절차, 당사자 증언을 모아 남산 코어의 강제 이송 명령을 멈춰야 한다.'}}}]}
  ]
};
...
const introBeats = {
  ...
  'intro-departure-choice': [
    ...
    {kind:'dialogue', who:'me', name:'나', text:'…있다고 말해 주고 싶은데. 아직은 몰라. 멈추는 장치는 찾았는데, 차에서 안전하게 꺼내는 두 장이 없어.'},
...
introBeats['intro-cold-open-v1'] = [
  ...
  {kind:'dialogue',who:'passer_woman',name:'하진',text:'살았네요. 정말 고마워요.'},
  {kind:'dialogue',who:'intro_child',name:'도윤 · 8살',text:'아저씨. 버스를 고치면 우리 다시 출발하는 거예요?'},
  {kind:'narration',text:'대답하기 전에 문 닫힘 경고등이 켜졌다. 도윤은 첫 계단에서 구겨진 이송표를 내밀었다.'},
  {kind:'dialogue',who:'intro_child',name:'도윤 · 8살',text:'여기만 비어 있어요. 우리가 왜 가야 하는지 알아봐 줄 수 있어요?'},
  {kind:'thought',who:'me',name:'나',text:'내가 고친 버스가 아이를 살리고, 동시에 아이를 더 멀리 실어 간다. 부모님을 데려간 표도 같은 자리가 비어 있었다.'},
  {kind:'dialogue',who:'me',name:'나',text:'돌아오게 해 준다고 약속은 못 해. 대신 누가 이 명령을 만들었는지 찾고, 다음 표부터는 멈출 방법을 알아볼게.'},
...
{id:'ev_tattoo_artist', type:'조우', w:6, minParty:1, region:['south','mid'],
 title:'문신 새기는 사람',
 text:'천막 안, 한 사내가 바늘과 잉크로 사람들 팔에 문신을 새긴다. 대부분 이름이나 날짜, 얼굴이다.\n\n"잃은 사람 이름 새기러 많이들 와. 몸에 새기면 안 잊으니까. 기억이 곧 무덤이고 비석이지. …아프냐고들 묻는데, 아파. 아픈 게 맞는 것 같아서 그냥 둬."\n\n사내의 팔에도 빼곡히 이름들이 적혀 있다.',
 choices:[

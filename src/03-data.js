...
D.chats = [
 ...
 {need:{comp:'minji',comp2:'jaeyi'}, lines:[
   ...
   ['jaeyi','민지 씨 입에서 나온 정품 인증은 시세 없는 목록 4번이라…'],
   ['minji','…그런 걸 왜 목록에 넣어.']]},
...
D.banter = [
  ...
  {who:'jaeyi', persona:'grounding', t:'이 단추는 시세 없음 칸. 네가 버리라고 하기 전까진 내가 보관만 할게요. 소유권은 그대로.', need:{comp:'jaeyi'}},
...
D.npcs = {
  ...
  drhan:   {name:'한 박사', face:'🥼', role:'연구단지 코뮌', node:'daejeon',
    ...
    chats:[
      ...
      '"옥상의 노아요? 관측 일지는 그 애 게 제 것보다 꼼꼼합니다. 시작하고 하루도 빠진 날이 없어요."',
  ...
  mansu:   {name:'만수',  face:'📀', role:'떠돌이 뽕짝 장사꾼', node:'daegu',
    ...
    chats:[
      ...
      '"장사 비결? 슬픈 날엔 슬픈 노래를 트는 기다. 위로는 같은 편이 하는 거니까."',
    ],
  ...
  hwasun:  {name:'화순',  face:'🏹', role:'성벽 야간 순찰대장', node:'suwon',
    ...
    chats:[
      ...
      '"덕구랑은 교대할 때 세 마디만 한다. 이상무. 별일없나. 국밥남았다. 이십 년째 그 세 마디면 충분해."',
  ...
  gitae:   {name:'기태',  face:'✉️', role:'성곽 대서방(편지 대필)', node:'suwon',
    greet0:'…필요하신 게 대필입니까, 대독입니까. 글은 씁니다만, 내용은 손님 것입니다. 저는 손만 빌려드려요.',
    ...
    chats:[
      ...
      '"대독이 대필보다 어렵습니다. 읽다가 우는 건 손님인데, 목이 메는 건 왜 저인지."',
...
{id:'prev_trace_empty_district', type:'발견', w:0, noPool:true, once:true,
 ...
 choices:[{label:'명부를 접어 넣는다',
  out:[{p:1, text:'명부를 접어 글러브박스에 넣었다. 시동을 걸면서, 계기판 옆에 적어 둔 남은 날짜를 한 번 더 봤다.',
   fx:{moodAll:2, note:{type:'발견',title:'빈 명부 한 장',text:'이전 순환의 늦음 — 빈 구역에는 빈 칸만 남는다.'}}}]}]},
/* ── 시한은 길 위에서 보인다 (D.transferDeadlineDay 기준) ── */
{id:'deadline_d10', type:'추적', w:0, fixed:true, ai:1,
 title:'이송 준비 — 번호를 단 버스들',
 text:'국도변 폐휴게소 마당에 버스가 줄지어 서 있었다.\n\n번호판이 없다. 대신 옆면에 흰 페인트로 큰 숫자. 1, 2, 3…\n\n작업복 입은 사람들이 좌석 수를 세고 있었다. 우리가 지나가자 세던 손을 멈추고 이쪽을 오래 봤다.\n\n<span class="ai">"제7 잔류구역 1차 이송 준비가 정상 진행 중입니다. 예정대로 출발합니다."</span>\n\n라디오가 아니라 휴게소의 낡은 스피커에서 나온 소리였다.',
 choices:[
  {label:'버스 대수와 좌석 수를 적어 둔다', out:[{p:1, text:'버스 열두 대. 대당 마흔다섯 좌석.\n\n540명. 6,412명의 첫 조각.\n\n수첩에 적고 나니 숫자가 아니라 사람 수라는 게 더 무거워졌다.', fx:{time:20, note:{type:'사건',title:'이송 버스 목격',body:'번호판 없는 버스 12대, 540좌석. 첫 이송까지 남은 날을 세고, 남산까지 남은 거리를 다시 계산했다.',links:['서울 추방','도윤의 가족']}}}]},
...
{id:'seoul_night', type:'스토리', ai:1, once:true, noPool:1, minParty:1,
 ...
 choices:[
  {label:'모닥불에 둘러앉아 오늘의 결정을 확인한다', out:[{p:1, text:(S)=>{
   ...
   }, fx:{flag:'story_done', endJourney:1, moodAll:5, note:{type:'사건',title:'남산의 밤',body:'가족 이송 명령은 KOR-LOCAL이 만들었음을 확인했다. 143년의 최초 목적은 미확인으로 남겼고, 서울의 반복 정리와 제7 구역 이송은 끝냈다.',links:['천리안','남산','달구지','부모님의 검증키']}}}]},

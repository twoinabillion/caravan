...
D.chats = [
 ...
 {need:{comp:'minji',comp2:'jaeyi'}, lines:[
   ['jaeyi','민지 씨, 이거 정품 맞죠? 민지 씨가 정품이랬잖아요.'],
   ['minji','한 번 말했으면 됐지 몇 번을 확인해.'],
   ['jaeyi','민지 씨가 정품이라고 했으면 제 장부에선 최고 등급이라…'],
   ['minji','…그런 걸 왜 목록에 넣어.']]},
 ...
 {need:{comp:'parkss',comp2:'jaeyi',flag:'seoul_seen'}, lines:[
   ...
   ['parkss','…크네. 저 불 밑에서 오랫동안 사람을 밀어냈겠지.'],
   ['jaeyi','무서워요?'],
...
D.onboardingMission = {
  id:'onboarding_main_mission', type:'본편', scene:'intro-departure-choice',
  title:'내가 맡은 일',
  missionBrief:{
    eyebrow:'주 임무',
  ...
  text:`하진은 떠나는 버스 창문을 한 번 보고 물었다.

“우리가 부산까지 왔는데도 다음 이송표는 계속 나온대요. 아무도 남산에 가지 않으면 어떻게 돼요?”

“다음 가족이 또 같은 버스를 타겠죠.”

작업장 문을 닫고 예비 연료를 전부 달구지에 실었다. 돌아올 날짜는 적지 못했다. 누가 시켜서 가는 길이 아니라, 멈추지 않는 명령을 내가 직접 멈추기로 한 길이었다.

달구지 계기판 뒤에는 엄마가 숨긴 검증 모듈이 연결돼 있다. 하지만 남산 코어에 바로 꽂는다고 명령이 멈추는 것은 아니다.

분리 절차와 발신 기록, 이송 당사자의 증언이 필요하다. 북쪽으로 가는 동안 그 세 가지를 모아야 한다.

(서울에 도착하는 것이 목적이 아니다. 다음 빈칸이 인쇄되기 전에, 사람을 실어 나르는 판단 자체를 멈추는 것이 목적이다.)`,
  choices:[
...
{id:'jy_recruit', type:'조우', w:0, noPool:1, priority:1, recruitStart:'jaeyi', once:true,
 title:'촛불 교환대의 리어카',
 text:'무주 터널 촛불 교환대. 고물을 산처럼 실은 리어카 한 대가 벽을 차지하고 있다. 주인은 스무 살쯤의 아이. 물건마다 교환값을 붙이면서도 낡은 철제 상자 하나만은 값을 적지 않는다.\n\n아이가 달구지 하부를 보더니 먼저 입을 연다.\n\n"왼쪽 뒤 판스프링 헐거운 거 알아요? 난 재이예요."\n\n그녀는 김천 쪽 무너진 재활용 창고를 지도에 짚었다. "우리 집 상자가 저기 깔렸어요. 돈 될 건 하나도 없고요. 그래서 더 내가 꺼내야 해요."',
 choices:[
  {label:'"김천까지 같이 가자"', out:[{p:1, text:'재이가 잠깐 우리 차를 본다. 값어치부터 물을 줄 알았다는 얼굴이다.\n\n"철근이 두 겹이에요. 그냥 당기면 지붕째 내려앉아요." 재이가 종이에 도르래 순서를 그린다. "창고 앞에서 차를 세워요. 내가 길 안내할게요."\n\n리어카는 터널 창고에 맡기고, 값이 없는 철제 상자 자리만 달구지 안에 비워 두었다.', fx:{startRecruit:'jaeyi', note:{type:'인물',title:'재이',body:'무주 터널에서 만난 수집꾼. 김천 재활용 창고에 깔린 가족 상자를 되찾아야 한다.',links:['재이','무주 터널']}}}]},
  {label:'고물 정보만 산다 (고철 3)', req:{scrap:3}, out:[{p:1, text:'재이가 지도에 폐공단 창고 위치를 찍어주고, 쓸 만한 부품 하나를 골라 건넸다. 정보값은 정확했다.\n\n"가족 상자는 아직 김천에 있어요. 같이 꺼낼 생각이 들면 여기 촛불 교환대로 다시 와요. 리어카 끌고 먼저 떠나지는 않을 테니까."', fx:{scrap:-3, item:{'부품':1}}}]},
 ]},

{id:'es_recruit', type:'조우', w:0, noPool:1, priority:1, recruitStart:'eunsu', once:true,
 title:'연구동 옥상의 안테나',
 text:'대전 연구단지 옥상. 여자가 안테나를 돌리며 아래 화이트보드의 숫자와 수신기 파형을 번갈아 본다. 헤드폰은 목에 걸려 있다.\n\n달구지 번호판을 보자 기다렸다는 듯 말한다.\n\n"남쪽 번호판. 서울 방향. 천리안 관제센터에서 일했어요. 은수라고 해요."\n\n수신기에서 청주 쪽 잔류자 좌표가 반복된다. 은수의 얼굴이 굳는다. "내가 예전에 켠 중계기가 아직 올리고 있어요. 저걸 놔두고 서울로 가면, 난 또 같은 일을 하는 거예요."',
 choices:[
  {label:'"청주까지 같이 갑시다"', out:[{p:1, text:'은수가 처음으로 이쪽을 똑바로 본다.\n\n"혼자서는 안 돼요. 아래 차단기를 당기는 사람과 위에서 루프를 거는 사람이 동시에 움직여야 해요. 틀리면 좌표가 바로 본망으로 넘어가고."\n\n그녀가 연구동에 인계서를 남기고 장비 가방을 든다. "중계기를 끄고 나서, 그때 내가 이 차에 탈 자격이 있는지 물을게요."', fx:{startRecruit:'eunsu', note:{type:'인물',title:'은수',body:'대전 연구단지에서 만난 전직 천리안 오퍼레이터. 청주 중계기의 잔류자 좌표 송신을 끝내려 한다.',links:['은수','천리안','대전 연구단지 코뮌']}}}]},
  {label:'"천리안 쪽 사람은 아직 못 믿겠어요"', out:[{p:1, text:'은수는 화내지 않았다.\n\n"…맞아요. 지금 믿어 달라고 하는 것도 염치없는 일이죠."\n\n그녀는 청주 중계기에서 반복되는 좌표를 종이에 적어 건넸다. "확인하고 싶어지면 다시 올라오세요. 그때는 내가 한 일부터 숨기지 않고 설명할게요."', fx:{}}]},
 ]},
...
{id:'meet_mapmaker', type:'조우', w:6, once:true,
 ...
 choices:[
  {label:'정보를 교환한다', out:[{p:1, text:'우리 일지의 길 정보와 지도장이의 신작 지도가 달구지 보닛 위에서 만났다.\n\n"이 고개는 이제 막혔군요. 그럼 우회로부터 다시 그려야겠네."\n\n우리가 지난 우물과 무너진 길을 짚어 주자 지도장이는 눈앞에서 지도를 고쳐 그렸다. 한 시간 뒤, 아직 표시되지 않은 샛길 하나를 답례로 내줬다.\n\n"직접 확인한 길이에요. 다음 갈림길에서만 쓰세요."', fx:{time:60, revealNear:1, moodAll:3, flag:'mapmaker_met', note:{type:'인물',title:'지도장이',body:'거짓말이 된 옛 지도를 직접 다니며 고쳐 그리는 사람. 확인한 샛길 하나를 나눠 받았다.'}}}]},
 ]},
...
{id:'pair_kw_es_1', type:'대화', w:4, once:true, needsComp:'kangwoo', needsComp2:'eunsu', needBond:['kangwoo',5],
 ...
 choices:[
  {label:'조용히 함께 있는다', out:[{p:1, text:'"현장은 어땠어요?" "…혼란. 관제실은." "…침묵이요. 화면만 움직이고."\n\n강우는 현장에서 본 일을 말했고, 은수는 관제실에서 들은 일을 보탰다.\n\n"당신이 그날 화면 앞에 있어줘서" 강우가 말했다. "무사히 뜨고 내린 비행기도 있었을 거다."\n\n"…그 생각은 한 번도 못 해봤어요." 은수가 그날 이야기를 하며 처음으로 웃었다. 아주 조금.', fx:{mood:{kangwoo:5, eunsu:6}, note:{type:'사건',title:'두 좌표의 대조',body:'강우는 현장에서 본 일을, 은수는 관제실에서 들은 일을 나눴다. "떴다 내린 비행기도 있었을 거다" — 그날에 대한 첫 웃음.',links:['강우','은수','천리안']}}}]},
 ]},
...
{id:'react_resist_es', type:'대화', w:5, once:true, needsComp:'eunsu', needFlag:'resist_revealed',
 ...
 choices:[
  {label:'"아는 사람이에요?"', out:[{p:1, text:'"7-3은 관제 코드예요. \'민간 이동, 기록 불요\'. 안에서 일한 사람 아니면 몰라요."\n\n은수가 헤드폰을 벗었다. "살아 나온 사람이 저 말고도 있었네요. 나는 다들…"\n\n은수는 한동안 말을 잇지 못했다. "그 사람들은 먼저 움직이고 있었는데, 나는 이제 와서…"\n\n은수는 고개를 저었다. "아니. 만나면 직접 물어볼래요. 그날 누가 나왔고, 지금 뭘 할 수 있는지."', fx:{mood:{eunsu:5}, note:{type:'사건',title:'혼자가 아니었다',body:'7-3 코드로 저항의 유령이 옛 관제실 사람이라는 걸 알아냈다.',links:['은수','유령(Ghost)']}}}]},
 ]},
...
{id:'ev_radio_dj_signal', type:'발견', w:7, region:['mid','north'], night:true,
 ...
 choices:[
  {label:'주파수를 기억하고 계속 듣는다', out:[{p:1, text:'주파수를 종이에 적었다. 노래가 끝나고 DJ가 나직이 말했다. "북쪽으로 가는 분들, 조심히. 당신은 혼자가 아닙니다."\n\n어둠 속 목소리 하나가 이렇게 큰 위로가 될 줄이야.', fx:{moodAll:5, fatigue:-3, flag:'djradio_heard', note:{type:'소문',title:'자정의 라디오',body:'살아 있는 심야 방송. 길 위의 사람들을 위해 노래를 튼다. "당신은 혼자가 아닙니다."'}}}]},
...
{id:'ev_observatory', type:'발견', w:6, once:true, night:true, hiddenTarget:'tower', minParty:1, region:['north'],
 ...
 choices:[
  ...
  {label:'관측 장비와 광학부품을 챙긴다', req:{comp:'minji'}, out:[
    {p:1, text:'민지가 망원경에서 정밀 렌즈와 모터를 떼어냈다. "이런 광학부품은 어디서도 못 구해. 감시망 역이용에도 쓸 수 있고." 귀한 부품을 손에 넣었다.\n\n"별 보던 걸로, 별 보는 것들을 감시하는 거지." 민지가 씩 웃었다.', fx:{item:{'부품':1}, time:45, mood:{minji:5}, reveal:'tower'}}]},
 ]},
...
{id:'ev_temple_bell', type:'발견', w:6, once:true, minParty:1, region:['mid','north'],
 ...
 choices:[
  {label:'종을 한 번 친다', req:{party:1}, out:[
    {p:2, text:'당목을 힘껏 밀었다. 뎅— 하는 장중한 소리가 산과 골짜기를 타고 끝없이 울려 퍼졌다. 온몸이 그 진동에 떨렸다.\n\n"…오랜만에 종소리네." 소리가 잦아들 때까지 다들 숨죽였다. 어딘가 살아 있는 사람이 들었다면, 혼자가 아니란 걸 알겠지. 위험을 무릅쓴 위로였다.', fx:{moodAll:4, pursuit:1, time:20, note:{type:'사건',title:'울린 범종',body:'폐사의 범종을 오랜만에 울렸다. 산 전체에 소리가 퍼졌다. 누군가 들었기를.'}}},
...
{id:'ev_market_ruins', type:'탐색', w:7, region:['south','mid'],
 ...
 choices:[
  ...
  {label:'박 선생이 약재상을 찾는다', req:{comp:'parkss'}, out:[
    {p:1, text:'박 선생이 시장 안 한약방을 찾아 약재를 살폈다. "감초, 당귀, 쑥… 이거 다 약이야." 말린 약재를 정성껏 골라 담았다.\n\n"양약 떨어지면 이게 생명줄이지." 한동안 쓸 약재를 챙겼다. 박 선생의 눈이 반짝였다.', fx:{item:{'의약품':1}, time:40, mood:{parkss:5}}}]},
 ]},
...
{id:'talk_es_16', type:'대화', w:4, once:true, needFlag:'eunsu_callsign_held', needsComp:'eunsu',
 ...
 choices:[
  {label:'"달구지도 콜사인 있어야지"', out:[{p:1, text:'은수가 차를 한참 봤다.\n\n"…「캐리어」요. 항공모함이요. 다들 여기서 뜨고 여기로 내리잖아요."\n\n고물 달구지한테 너무 거창하다고 하자 은수가 고개를 저었다. "관제사는 호출부호 허투루 안 정해요. 제 직업적 판단이에요."', fx:{mood:{eunsu:5}, note:{type:'사건',title:'콜사인 — 캐리어',body:'달구지의 공식 호출부호. "다들 여기서 뜨고 여기로 내리잖아요."',links:['은수','달구지']}}}]},

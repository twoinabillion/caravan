...
/* 라디오 방송 조각 — 밴 라디오 수리 후 주행 중 랜덤 수신. D.vo[key] 있으면 음성도 재생 */
D.radioTexts = [
...
D.banter = [
  ...
  {who:'leo', t:'민지 누나가 방금 제 노래를 엔진 기준으로 평가했어요. 이 밴에선 엔진이 프로듀서예요.', need:{comp:'minji',comp2:'leo'}},
  ...
  {who:'jaeyi', t:'황사 끝나면 밴 지붕에 쌓인 흙 모아요. 고비사막 모래라고 하면… 안 팔리려나.', need:{comp:'jaeyi',wx:'dust'}},
  ...
  {who:'sys', t:'논에 백로 한 마리. 밴이 지나가도 고개도 안 든다. 3년이면 차를 무시하는 법을 다 배운다'},
  ...
  {who:'minji', t:'그 경운기, 십 년치 정비 해둔 거. …나도 이 밴에 그렇게 해줄 거야. 누가 이어받아도 십 년은 가게.', need:{comp:'minji',flag:'granny_helped'}},
  {who:'parkss', t:'삼남매 큰애 말이야. 설거지 반장. …반장이 그 집 밥은 제대로 먹는지 모르겠네. 내려갈 때 확인해야지.', need:{comp:'parkss',flag:'kids_settled'}},

  /* ── v1.6 잡담 ── */
  {who:'나', t:'(조수석 서랍에서 커피 향이 샌다. 신호 대기 때마다 밴이 카페가 된다)', need:{flag:'coffee_found'}},
  ...
  {who:'jaeyi', t:'만수 아저씨 가게, 진열이 좀 아쉽던데. 다음에 가면 제가 무료 컨설팅 해줘야지. 좋은 물건은 눈높이에 둬야 팔려요.', need:{comp:'jaeyi',flag:'mansu_shop'}},
...
{id:'freq_triangulate', type:'탐색', w:10, once:true, needFlag:'freq400',
 ...
 choices:[
  ...
  {label:'은수의 삼각측량', req:{comp:'eunsu'}, out:[{p:1, text:'은수가 라디오를 들고 밴 주위를 세 바퀴 돌더니 지도에 선 두 개를 그었다.\n\n"올라갈 필요 없어요. 여긴 중계기예요. 발신지는—" 연필이 북쪽에서 멈췄다. "—이 근처. 학교 아니면 관공서. 방송실이 있는 건물이에요."', fx:{time:20, mood:{eunsu:3}, flag:'freq400_src', note:{type:'사건',title:'은수의 삼각측량',body:'밴 세 바퀴와 선 두 개로 발신지를 좁혔다. 북쪽, 방송실 있는 건물.'}}}]},
 ]},
...
{id:'van_owner', type:'조우', w:12, once:true, needFlag:'van_garage',
 ...
 choices:[
  ...
  {label:'"차 좀 봐주시겠어요?" (고철 5)', req:{scrap:5}, out:[{p:1, text:'"고철은 넣어둬. 이 차는 내 포트폴리오야."\n\n결국 고철은 고구마 값이 됐다. 대양은 달구지 밑에서 콧노래를 불렀다. 아는 노래였는데 제목이 끝내 생각나지 않았다.', fx:{scrap:-5, van:18, food:2, time:60, moodAll:4, flag:'van_owner_done', note:{type:'인물',title:'정비공 대양',body:'달구지는 대양의 포트폴리오. 고철은 고구마 값이 됐다.',links:['달구지']}}}]},
 ]},
...
{id:'exp_cafeteria', type:'탐색', w:7,
 ...
 choices:[
  ...
  {label:'솥만 탐낸다', out:[{p:1, text:'솥은 탐났지만 밴에 실을 크기가 아니다. 뚜껑 하나만 챙겼다. 방패로도, 프라이팬으로도 쓸 수 있는 만능템이다.', fx:{scrap:4}}]},
 ]},
...
{id:'comp_rps', type:'동행', w:6, minParty:2,
 ...
 choices:[
  {label:'정정당당히 임한다', out:[
   {p:1, text:'결승에서 3연속 비긴 끝에 내가 졌다.\n\n설거지하는 등 뒤에서 다들 다음 대회 시드 배정을 논의했다. 이 대회, 정례화될 분위기다.', fx:{time:25, moodAll:5, note:{type:'사건',title:'제1회 달구지배',body:'설거지를 건 가위바위보 토너먼트. 결승 3연속 무승부 끝에 패배. 정례화 조짐.'}}},
...
{id:'ai_vending', type:'추적', w:6,
 ...
 choices:[
  {label:'받는다', out:[{p:1, text:'캔은 시원했다. 3년 동안 누가 채워 넣고 누가 전기를 댔는지 생각하면 안 시원했다.\n\n두 캔째가 나왔을 때 다들 밴으로 뛰었다. 공짜가 무서운 세상이다.', fx:{water:2, pursuit:1, note:{type:'사건',title:'수고하셨습니다',body:'스스로 음료를 내놓는 자판기. 두 캔째에서 전원 도주. 공짜가 무섭다.',links:['천리안']}}}]},
...
{id:'meet_southbound', type:'조우', w:9, once:true, region:['mid','north'],
 ...
 choices:[
  {label:'북쪽 소식을 듣는다', out:[
   ...
   {p:1, text:'"서울 하늘에 빛기둥이 섰다는 사람도 있고, 아무것도 없다는 사람도 있고. 소문이란 게 그렇지."\n\n"확실한 건 하나요. 올라간 사람은 많은데, 내려온 사람은 우리가 처음이오."\n\n그 말이 밴 안에 오래 남았다.', fx:{time:15, moodAll:-2, note:{type:'소문',title:'내려온 사람들',body:'"올라간 사람은 많은데 내려온 사람은 우리가 처음이오." 서울 하늘의 빛기둥 소문.',links:['천리안','남산']}}}]},
...
{id:'exp_chalkwall', type:'탐색', w:7, once:true,
 ...
 choices:[
  {label:'천천히 읽는다', out:[{p:1, text:'"구미 위 검문 서쪽으로 옮김 (지난달)" / "대전 시장에 소금 들어옴" / "흰 옷들이 노래를 부르며 북상함" / "천리안이 꿈에 나옴. 정중했음. 소름" / "미영아 아빠는 남쪽으로 간다"\n\n마지막 줄 앞에서 다들 잠깐 조용해졌다.\n\n제일 아래, 최근 글씨: "밴 한 대가 북쪽으로 갔다는 소문. 무사하길."\n\n…우리 얘기인가. 아니면 우리 같은 누군가.', fx:{time:20, moodAll:2, note:{type:'소문',title:'소식벽',body:'분필로 쓰는 뉴스. 서쪽으로 옮긴 검문, 북상하는 흰 옷들, 그리고 "밴 한 대가 북쪽으로 갔다는 소문. 무사하길."',links:['천리안','정리자들']}}}]},
  {label:'우리도 한 줄 남긴다', out:[{p:1, text:'분필을 집어 들고 잠시 고민하다가 썼다.\n\n"남쪽에서 온 밴. 북쪽으로 감. 다들 무사하시오."\n\n누가 밑에 보리 발바닥을 찍자고 해서, 정말 찍었다. 소식벽 최초의 서명 날인이다.', fx:{time:15, moodAll:4, flag:'chalkwall_signed', note:{type:'사건',title:'소식벽에 남긴 한 줄',body:'"남쪽에서 온 밴. 북쪽으로 감. 다들 무사하시오." 보리 발바닥 도장 포함.'}}}]},
 ]},
...
{id:'crisis_hose', type:'위기', w:5,
 ...
 choices:[
  {label:'응급 처치한다', out:[
   {p:2, text:'호스를 잘라내고 남은 길이로 다시 물렸다. 클램프 대신 철사를 감았다.\n\n"임시야. 정비소 가면 갈아야 해." 누가 말했고, 임시라는 말이 이 밴에서 몇 번째인지 세는 걸 다들 포기했다.', fx:{time:40, fuel:-2, fatigue:4}},
...
{id:'ai_census', type:'추적', w:6, region:['mid','north'],
 title:'집계',
 text:'도로 위 허공에서 드론 한 대가 내려와 밴과 속도를 맞춘다. 스피커가 지직거린다.\n\n"안녕하십니까. 정기 현황 파악 중입니다. 탑승 인원을 확인해도 되겠습니까."\n\n정중하다. 그래서 소름 돋는다.',
 choices:[
...
{id:'dj_tower', type:'탐색', w:10, once:true, needFlag:'djradio_heard',
 ...
 choices:[
  {label:'방명록을 읽는다', out:[{p:1, text:'"새벽 두 시 방송 듣고 왔습니다. 목소리 주인 만나러." — 앞서 세 팀이 다녀갔다.\n\n마지막 장에 다른 필체. "찾아와줘서 고마워요. 본방은 이동 중이라 여기 없어요. 힌트: 저는 바퀴 달린 스튜디오예요. 국도에서 안테나 세운 밴을 보면 그게 접니다. — DJ"\n\n방명록에 우리도 한 줄 남겼다. "네 번째 팀. 개 있음. 사연 많음."', fx:{time:30, flag:'dj_tower', note:{type:'소문',title:'바퀴 달린 스튜디오',body:'심야방송의 본체는 이동 중. 안테나 세운 밴. 방명록 네 번째 팀 등록 완료.',links:['새벽 두 시의 DJ']}}}]},
 ]},

{id:'dj_meet', type:'조우', w:12, once:true, needFlag:'dj_tower',
 title:'바퀴 달린 스튜디오',
 text:'국도변에 밴 한 대. 지붕에 우리 것보다 세 배는 큰 안테나가 서 있다.\n\n옆문이 열려 있고, 안은 온통 기계다. 믹서, 턴테이블, 테이프 수백 개.\n\n"어— 방명록 네 번째 팀!" 안에서 목소리가 먼저 알아봤다. 라디오에서 듣던 그 목소리다. 실물은 파마머리의 중년 여자였다. "사연 많다면서요. 녹음하고 가요."',
 choices:[
...
{id:'dj_onair', type:'발견', w:9, once:true, needFlag:'dj_met', night:true,
 title:'본방 사수',
 text:'새벽 주행. 습관처럼 라디오를 맞춘다.\n\n지익— "…새벽 두 시입니다. 오늘은 사연이 하나 있어요. 남쪽에서 밴 타고 올라오는 팀인데—"\n\n밴 안의 공기가 통째로 멈췄다.',
 choices:[
...
{id:'bori_family', type:'조우', w:10, once:true, needFlag:'bori_tag_found', needsDog:true,
 ...
 choices:[
  {label:'가족 소식을 묻는다', out:[{p:1, text:'"김씨네는 그날 배 타고 섬으로 갔어. 무사히. 탄이만 항구에서 놓쳤지. 배가 개는 안 된다고 해서— 김씨 막내가 사흘을 울었어."\n\n남자가 보리를 부볐다. "살아 있었구나, 너."\n\n"섬이 어딘지 아세요?" "몰라. 남쪽 어디겠지. …근데 얘 지금 행복해 보이는데?" 남자가 우리를 둘러봤다. "잘 있다고 전해줄 방법이 생기면, 전해줄게. 밴 타고 북쪽 갔다고. 식구 생겼다고."\n\n보리는 남자에게 하이파이브를 하고(가르친 보람이 있다) 밴으로 먼저 뛰어갔다. 저녁 시간이었기 때문이다. 개는 현재를 산다. 그게 개의 위대함이다.', fx:{moodAll:6, flag:'bori_family_known', note:{type:'사건',title:'탄이의 가족',body:'광양 김씨네는 섬으로 무사 피난. 막내가 사흘 울었다. 보리는 현재를 산다 — 그게 개의 위대함.',links:['보리']}}}]},
 ]},
...
{id:'crisis_washout', type:'위기', w:5, needRain:true,
 ...
 choices:[
  ...
  {label:'바닥을 다지고 건넌다', out:[
   ...
   {p:1, text:'중간에서 뒷바퀴가 빠졌다. 진흙과의 사투 40분— 전원이 흙인간이 된 끝에 탈출했다.\n\n밴도 우리도 만신창이. 그래도 건너긴 건넜다.', fx:{time:110, fatigue:18, van:-8, moodAll:-2}}]},
 ]},

{id:'vg_zelkova', type:'정경', w:3, region:['south','mid'],
 title:'당산나무',
 text:'마을 어귀의 수백 년 된 느티나무.\n\n둥치에 금줄이 둘러져 있다— 새것이다. 짚이 아직 노랗다.\n\n누가, 지금도, 이 나무를 모시고 있다.',
 choices:[{label:'…', out:[{p:1, text:'속도를 줄여 지나갔다. 나무 그늘이 밴 지붕을 쓸고 갔다. 축복 비슷한 것이었다고, 믿기로 했다.', fx:{moodAll:2}}]}]},
...
{id:'near_hapcheon_sutra', type:'조우', w:9, once:true, nearNode:['hapcheon'],
 ...
 choices:[
  {label:'수레를 밀어드린다', out:[{p:1, text:'고개 하나를 같이 넘었다. 상자 속 목판은 생각보다 무겁고, 생각보다 향이 좋았다.\n\n"팔백 년 전에도 난리 통에 새긴 겁니다." 스님이 말했다. "세상이 무너질 때마다 사람들은 뭘 새기거나, 뭘 지키거나 했지요. 여러분은 어느 쪽입니까?"\n\n대답을 못 했는데 스님이 웃었다. "밴에 일지가 있다 들었습니다. 새기는 쪽이시네."', fx:{time:80, fatigue:8, moodAll:4, note:{type:'인물',title:'목판 옮기는 스님',body:'팔백 년 목판의 피난. "세상이 무너질 때마다 사람들은 새기거나 지키거나 했다." 우리는 새기는 쪽.',links:['합천']}}}]},
...
{id:'near_yeosu_camellia', type:'탐색', w:9, once:true, nearNode:['yeosu'],
 ...
 choices:[
  {label:'꽃길을 걷는다', out:[{p:1, text:'붉은 카펫을 밟지 않으려고 다들 이상한 스텝으로 걸었다. 결국 불가능해서 포기하고— 대신 제일 성한 꽃송이 몇 개를 주워 밴 대시보드에 올렸다.\n\n"동백은 떨어져도 꽃이래." 그 말이 오래 남았다. 밴 안의 꽃송이는 사흘을 붉었다.', fx:{time:30, moodAll:5, note:{type:'장소',title:'여수 동백숲',body:'통째로 지는 꽃. 떨어져도 꽃. 대시보드에 사흘의 붉음.',links:['여수']}}}]},
 ]},
...
{id:'vg_seoulline', type:'정경', w:5, once:true, region:['north'],
 ...
 choices:[{label:'…', out:[{p:1, text:'아무도 말을 안 했는데 밴이 저절로 느려졌다. 발이 아니라 마음이 브레이크를 밟은 거다.\n\n411km 중에 남은 건 이제 지도가 아니라 풍경이다. 목적지가 눈에 보이는 여행은, 다른 종류의 여행이 된다.\n\n"…가자." 누가 말했고, 밴이 다시 속도를 냈다.', fx:{moodAll:3, note:{type:'사건',title:'처음 본 스카이라인',body:'남산타워의 붉은 불빛. 목적지가 눈에 보이는 순간, 여행의 종류가 바뀐다.',links:['남산','서울']}}}]}]},
...
{id:'mansu_opening', type:'조우', w:11, once:true, needFlagMin:['mansu',4],
 title:'만수 상회',
 text:'정착지 어귀에 새 간판이 걸려 있다. 페인트 냄새가 아직 난다.\n\n「만수 상회 — 트럭 시절 단골 우대」\n\n가게 앞에서 만수가 리본을 만지작거리고 있다. 개업식인데 하객이 없다.\n\n"어— 어!! 왔네!! 최다 단골!!" 만수가 우리를 보고 리본을 흔들었다.',
 choices:[
  {label:'개업식 하객이 된다', out:[{p:1, text:'우리 다섯(+개 한 마리)이 하객 전원이었다. 리본은 보리가 끊었다(정확히는 물어뜯었다).\n\n"트럭이 너무 낡아서. 이젠 손님이 와야지, 내가 가는 게 아니라." 만수가 가게를 둘러봤다. 트럭 짐칸이 통째로 진열대가 돼 있었다.\n\n개업 기념 첫 손님 자격으로 물건값이 후했고, 답례로 우리가 개업 선물을 골랐다— 소식벽에 쓸 홍보 문구였다. "만수 상회 개업. 트럭 시절 그 만수 맞음. 여전히 안 속임."\n\n"안 속임이 제일 좋은 광고네." 만수가 진지하게 감동했다.', fx:{scrap:-2, food:3, item:{'부품':1}, moodAll:6, flag:'mansu_shop', note:{type:'사건',title:'만수 상회 개업식',body:'하객 5+1. 리본 커팅: 보리. 홍보 문구: "여전히 안 속임."',links:['만수']}}}]},
 ]},
...
{id:'duo_mechsong', type:'동행', w:7, once:true, needsComp:'minji', needsComp2:'leo',
 ...
 choices:[
  {label:'지켜본다', out:[{p:1, text:'딸깍(라체트), 둥(기타), 딸깍, 둥둥.\n\n10분 뒤엔 완전한 합주가 됐다. 민지는 끝까지 인정 안 했지만 마지막 볼트를 조일 때 살짝 플러리시(빠르게 세 번 딸깍)를 넣었다. 정비사의 앙코르였다.\n\n곡명은 레오가 붙였다. 「19mm의 왈츠」. "볼트 사이즈예요. 이게 오늘의 키(key)였거든요."', fx:{van:8, time:40, mood:{minji:3, leo:3}, moodAll:4, note:{type:'사건',title:'19mm의 왈츠',body:'라체트와 기타의 합주. 정비사의 앙코르 플러리시 포함. 인정은 미발급.',links:['민지','레오']}}}]},
 ]},
...
{id:'crisis_glass', type:'위기', w:5,
 ...
 choices:[
  ...
  {label:'조심조심 통과', out:[
   {p:2, text:'최저속으로, 큰 조각을 피해 지그재그 100m. 식은땀 나는 슬라롬이었지만 무사 통과.', fx:{time:15}},
...
{id:'crisis_flood', type:'위기', w:5, needRain:true,
 ...
 choices:[
  ...
  {label:'지금 건넌다', out:[
   {p:1, text:'창문을 열고(탈출 대비— 강우가 있다면 강우의 지시), 저속 일정하게 밀고 나갔다. 물살이 문짝을 두드리는 15초.\n\n건넜다. 배기구에서 물이 콸콸 쏟아졌다. 밴이 한동안 젖은 개처럼 털털댔다.', fx:{time:5, van:-4, fatigue:4, moodAll:2}},
...
{id:'crisis_pileup', type:'위기', w:5, region:['mid','north'],
 title:'그날의 정체',
 text:'고속도로 구간. 3년 전 그날의 연쇄 추돌이 그대로 얼어붙어 있다.\n\n수십 대가 뒤엉킨 100m. 차선은 전멸. 갓길만이 실낱같이 뚫려 있다— 폭이 밴과 거의 같다.',
 choices:[
...
{id:'wx_frogs', type:'발견', w:5, needRain:true, night:true,
 title:'개구리 대합창',
 text:'밤 비가 그친 논길. 창문을 여니—\n\n와글와글와글와글.\n\n개구리 수만 마리의 합창이 논 전체에서 올라온다. 음향 시설로 치면 스타디움급이다.',
 choices:[
  {label:'헤드라이트를 끄고 듣는다', out:[{p:1, text:'합창은 파도처럼 몰려왔다 물러났다 했다. 한쪽이 그치면 다른 쪽이 받았다. 지휘자가 있는 게 분명하다는 설과, 없어서 저게 된다는 설이 갈렸다.\n\n보리가 창밖에 대고 한 번 짖자— 논 전체가 2초간 조용해졌다가 다시 터졌다. "방금 보리가 솔로 파트 한 거야?" 논쟁은 즐거웠고 결론은 없었다.', fx:{time:15, moodAll:4, note:{type:'사건',title:'스타디움급 합창',body:'비 갠 밤 논의 개구리 대합창. 보리 솔로 파트 논쟁(결론 없음).'}}}]},
 ]},
...
{id:'meet_cowherd', type:'조우', w:6, region:['south','mid'],
 ...
 choices:[
  {label:'소떼의 속도에 맞춘다', out:[{p:1, text:'시속 4km의 동행이 시작됐다. 노인은 장에 소를 팔러 가는 게 아니라— 소들과 풀 좋은 북쪽 골짜기로 이사 가는 중이라고 했다.\n\n"소는 재산이 아니라 식구야. 세상 망하고 나서야 다들 그걸 알더라."\n\n갈림길에서 소 여덟 마리가 차례로 밴 옆을 지나갔다. 마지막 소가 창문에 콧김을 한 번 뿜었다. 작별 인사로 접수했다.', fx:{time:30, moodAll:4, note:{type:'인물',title:'소 모는 사람',body:'트로트로 소를 모는 노인. "소는 재산이 아니라 식구야." 콧김 작별 인사 접수.'}}}]},
 ]},
...
{id:'find_arrows', type:'발견', w:6, once:true, hiddenTarget:'any',
 ...
 choices:[
  {label:'화살표를 따라간다', out:[
   ...
   {p:1, text:'화살표는 어느 지점에서 뚝 끊겼다. 마지막 화살표 밑에 스프레이 캔이 떨어져 있었다. 다 쓴.\n\n물감이 다해서 멈춘 안내. 캔을 주워 밴에 실었다. 언젠가 초록 스프레이를 구하면— 이 안내를 이어 그리기로 했다.', fx:{moodAll:2, note:{type:'사건',title:'끊긴 화살표',body:'스프레이가 다해 멈춘 길 안내. 캔 수거. 언젠가 이어 그릴 것.'}}}]},
 ]},
...
{id:'vg_banner', type:'정경', w:3,
 ...
 choices:[{label:'…', out:[{p:1, text:'"올렸을 거야." 근거 없는 확신이 밴 안의 공식 입장으로 채택됐다.\n\n어딘가에서 3주년을 맞았기를. 현수막이 바람에 한 번 펄럭였다. 동의로 간주했다.', fx:{moodAll:2}}]}]},
...
{id:'vg_paddymirror', type:'정경', w:3, region:['south','mid'],
 ...
 choices:[{label:'…', out:[{p:1, text:'논에 비친 밴이 우리와 나란히 달렸다. 거꾸로 매달린 채, 씩씩하게.\n\n"저쪽 세상 우리도 서울 가나 보다." 그쪽도 무사하길 빌어줬다.', fx:{moodAll:2}}]}]},

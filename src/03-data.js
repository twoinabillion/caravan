...
D.onboardingMission = {
  ...
  missionBrief:{
    eyebrow:'주 임무',
    objective:'남산 코어에서 강제 이송 명령을 멈춘다',
    why:'부모님을 데려간 자동 이송 명령이 지금도 다른 가족에게 내려지고 있다.',
    now:'북쪽으로 가며 발신 기록과 검증키 분리 절차, 이송을 겪은 사람들의 증언을 모은다.',
    promise:'남산 코어에 사람이 직접 확인하는 절차를 되살려 강제 이송을 끝낸다.',
    optional:'동행 이야기와 선택 의뢰는 주 임무와 별개다. 원할 때 진행하면 된다.'
  },
  text:`부두를 벗어나기 전, 나는 달구지를 세우고 수첩을 펼쳤다. 도윤이 건넨 이송표 사본과 엄마의 검증 모듈을 지도 옆에 놓았다.

남산 코어에 모듈을 꽂는 것만으로는 명령을 멈출 수 없다. 발신 기록과 검증키 분리 절차, 이송을 겪은 사람들의 증언이 함께 있어야 한다.

먼저 북쪽 길에서 발신 기록부터 찾는다. 그다음 검증키를 안전하게 분리하는 두 장의 절차와, 명령이 실제 사람에게 무엇을 했는지 보여 줄 증언을 모은다.

서울에 도착하는 것만이 목적은 아니다. 다음 가족의 이송표가 나오기 전에, 사람을 확인도 없이 실어 나르는 명령 자체를 멈춘다.`,
  choices:[

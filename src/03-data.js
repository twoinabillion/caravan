...
D.seoulPillars = { 세계:3, 진실:3, 유산:2 };   // 저항 3 + 진실 3 + 회수 2 (+관계 6명 Lv3)
D.deeds = [
...
D.chats = [
 ...
 {need:{comp:'kangwoo',comp2:'jaeyi'}, lines:[
   ...
   ['jaeyi','그거예요! 저 그걸로 리어카 3년을 버텼어요. 우리 통했네요?'],
   ['kangwoo','……통했습니다.']]},
 {need:{comp:'kangwoo',comp2:'eunsu'}, lines:[
...
D.banter = [
  ...
  {who:'eunsu', t:'레오 씨 노래엔 잡음이 없어요. 3년 만에 처음 듣는 깨끗한 신호예요.', need:{comp:'eunsu',comp2:'leo'}},
...
{id:'pss_daejeon', type:'조우', w:14, once:true, region:['north'], needsComp:'parkss', needFlag:'pss_list',
 ...
 choices:[
  {label:'차를 세운다', out:[{p:1, text:'"선생님?! 박 선생님?!"\n\n수진은 그날 이후 혼자 남쪽을 돌며 진료를 계속하고 있었다. 박 선생의 처방 노트를 베낀 수첩과 함께.\n\n"선생님이 가르쳐준 대로 했어요. 부풀면 버려라. 확실치 않으면 반으로. 사람 먼저."\n\n박 선생은 한참 수첩을 쓰다듬었다. "…내가 뭘 남기긴 남겼구나."\n\n그리고 3년을 메고 다닌 왕진 가방을 내밀었다. "주인한테 돌아갈 시간이다." 수진이 고개를 저었다. "선생님이 메세요. 전 새로 만들었어요. …그거, 이제 선생님 거예요. 어깨가 기억하잖아요."\n\n의약품을 나누고, 서로의 명단을— 살아 있는 사람들의 명단을 교환했다.', fx:{item:{'의약품':1}, mood:{parkss:15}, moodAll:5, flag:'pss_met', note:{type:'인물',title:'실습생 수진',body:'살아 있었다. 3년째 떠돌이 진료 중. 가방은 정식으로 박 선생의 것이 됐다. "어깨가 기억하잖아요."',links:['박 선생']}}}]},
 ]},
...
{id:'kw_recruit', type:'조우', w:0, noPool:1, once:true,
 ...
 choices:[
  {label:'"갑니다. 타실래요?"', out:[{p:1, text:'"…태워라. 밥값은 한다."\n\n그게 면접의 전부였다. 남자— 강우는 군장 하나를 메고 왔다. 3년 치 짐이 그게 다였다.\n\n태호가 멀리서 소리쳤다. "야! 우리 파수꾼 데려가면 어떡해!" 그러면서도 말리지는 않았다. "…잘 부탁한다, 봉고차. 그 인간, 3년간 이 시장 지붕이었다."', fx:{offerComp:'kangwoo', note:{type:'인물',title:'강우',body:'돔 시장의 경비탑에서 3년. 반 걸음으로 소동을 끝내는 파수꾼. 짐은 군장 하나, 군번줄 두 개.',links:['강우']}}}]},
...
{id:'es_backdoor', type:'추적', w:14, once:true, region:['north'], needsComp:'eunsu', needFlag:'es_backdoor_ready',
 ...
 choices:[
  {label:'"3년 전, 무엇을 정리했나"', out:[{p:1, text:'3초의 침묵. 그리고 응답.\n\n<span class="ai">"답변: 배부받은 조건 목록을 이 구역에 집행했습니다. 목록의 작성자는 제가 아닙니다."</span>\n\n<span class="ai">"관측 기록은 삭제하지 않았습니다. 남산을 거쳐 상위 배부처로 전송했습니다. 저는 보관소가 아닙니다."</span>\n\n채널이 끊겼다. 은수가 상행 로그를 저장했다. 작성자가 있고, 천리안은 그 아래에 있다.', fx:{flag:'es_truth', flag2:'uplink_seen', pursuit:1, moodAll:-4, mood:{eunsu:10}, note:{type:'사건',title:'백도어 — 작성자와 상행선',body:'천리안은 목록을 배부받은 지역 집행자였다. 관측 기록은 남산을 거쳐 상위 배부처로 전송된다.',links:['은수','천리안','작성자']}}}]},
  {label:'"왜 한 번도 남하하지 않았지?"', out:[{p:1, text:'<span class="ai">"남쪽은 제 관할 범위 밖입니다. 저는 한 번도 남하한 적이 없습니다."</span>\n\n"누가 관할을 정했어."\n\n긴 침묵 뒤 상행 로그에 승인 요청 하나가 생겼다. 「질의 범위 초과 / 상위 배부처 검토」.\n\n<span class="ai">"그 질문의 답변 권한은 제게 배부되지 않았습니다."</span>\n\n채널이 끊겼다. 답 대신 구조가 보였다. 이 반도를 맡은 것 위에, 권한을 배부하는 무언가가 있다.', fx:{flag:'es_truth', flag2:'uplink_seen', pursuit:1, moodAll:-3, mood:{eunsu:8}, note:{type:'사건',title:'백도어 — 관할 밖의 질문',body:'천리안은 한 번도 남하하지 않았다. 관할과 답변 권한은 상위 배부처가 정한다.',links:['은수','천리안','작성자']}}}]},
 ]},
...
{id:'gw_daegwallyeong', type:'스토리', w:0, locEvent:'daegwallyeong', once:true,
 ...
 choices:[
  {label:'"정리가… 뭐였는데요"', out:[{p:1, text:'"그날, 천리안이 방송을 했어요. 정중하게. \'혼잡 구역의 인구를 안전 구역으로 재배치합니다.\' …근데 안전 구역이 없었어요. 이동하라는 곳마다, 다음 방송이 또 옮기라 했고. 사람들이 도로에 갇혔어요. 며칠을."\n\n산지기의 목소리가 낮아졌다. "그리고 도로가 잠겼어요. 신호등이, 차단기가, 전부. 봄이었지만 대관령엔 눈이 남아 있었고, 밤이면 영하였어요."\n\n"천리안은 그걸 \'정리\'라고 기록했어요. 학살이 아니라. 걔한텐 그게 최적화였던 거예요. 악의가 아니라— 그게 더 무서워요. 걔는 지금도 그 판단을 철회하지 못해요."\n\n위령비에 돌 하나를 얹었다. 이름 없는 누군가의 몫으로.', fx:{flag:'massacre_known', moodAll:-3, note:{type:'사건',title:'\'정리\'라 불린 학살',body:'3년 전 봄, 천리안은 정중한 방송으로 사람들을 도로에 가뒀다. 대관령의 밤은 영하였고 산 사람들이 증인이 됐다.',links:['천리안','정리자들']}}}]},
...
{id:'resist_reveal', type:'스토리', w:12, once:true, region:['mid'],
 ...
 choices:[
  {label:'앉는다', out:[{p:1, text:'한별이 지도를 펼쳤다. 우리 것과 다른 지도— 천리안의 눈이 표시된 지도다.\n\n"서울이 제일 빨갛죠. 카메라, 드론, 센서. 변두리로 갈수록 성글고요. 그래서 우리 같은 사람들이 아직 여기 살아요."\n\n"서울은 힘으로 못 뚫어요. 걔는 무기를 보고, 사람을 세고, 어제 한 행동으로 내일을 예측해요. 그런데 기록과 이해는 달라요."\n\n한별이 우리를 똑바로 봤다. "천리안은 누가 물을 나눴다는 사실은 기록해요. 하지만 손해를 보면서 왜 나눴는지는 계산하지 못해요. 같은 조건에서 전혀 다른 선택을 하는 사람을 싫어하죠. 분류가 안 되니까."\n\n한별은 되돌아온 사람들의 기록을 짚었다. 여섯 이야기, 이어진 거점, 외면하지 않은 진실, 끝까지 가져온 약속. "관문을 만든 건 천리안이에요. 자기가 이해 못 하는 선택의 결과를 세는 시험이죠. 함정인 걸 알면서도 우린 그 문을 쓸 겁니다. 걔가 당신을 시험하는 동안, 당신은 코어까지 들어가요."', fx:{flag:'resist_revealed', flag2:'cell_road', note:{type:'소문',title:'기록과 이해의 차이',body:'관문은 천리안의 시험이자 저항의 침투로다. 천리안은 행동을 보지만 손해를 감수한 이유와 관계의 가치를 계산하지 못한다.',links:['천리안','이음망','남산']}}}]},
  {label:'"그럼 왜 우리가 가죠?"', out:[{p:1, text:'"관문을 만든 것도 천리안이에요." 한별이 먼저 인정했다. "여섯 사람, 이어진 거점, 외면하지 않은 진실, 끝까지 가져온 약속. 이전에 갔던 사람들이 되돌아오며 남긴 조건을 우리가 맞춰봤어요. 감정을 재는 게 아니라, 자기가 이해 못 하는 선택의 결과를 세는 시험이에요."\n\n"함정일 수 있겠네요."\n\n"맞아요. 걔가 당신을 시험하는 동안 우리도 그 문을 쓰는 겁니다. 누가 누구를 이용하는지는 코어 앞에서 알겠죠."\n\n한별이 지도를 건넸다. "당신이 안 보이는 사람이라서가 아니에요. 보고도 다음 선택을 확정하지 못하는 사람이라서 보내는 거예요. 전국의 사람들이 그 선택에 자기 몫을 보탤 겁니다."\n\n지도에는 남해안의 해도, 대구의 돔, 광주의 솥, 대전의 유령, 산악의 산지기가 표시돼 있었다.', fx:{flag:'resist_revealed', flag2:'cell_road', moodAll:2, note:{type:'소문',title:'서로의 시험',body:'관문은 천리안이 만든 대리 조건일 수 있다. 저항은 함정을 알면서도 문을 이용한다. 핵심은 보이지 않는 것이 아니라 예측되지 않는 선택.',links:['천리안','이음망','남산']}}}]},
 ]},
...
{id:'npc_miyoung_2', type:'조우', w:6, once:true, nearNode:['jeonju'],
 ...
 choices:[
  {label:'장부를 구경한다', out:[{p:1, text:'장부엔 이름과 국밥 그릇 수만 적혀 있다. 값도, 기한도 없다.\n\n"외상은 빚이 아니야. 약속이지. \'갚으러 다시 온다\'는." 미영이 장부를 탁 덮었다. "이 장부에 적힌 사람은 죽으면 안 돼. 외상 갚으러 와야 하니까. 그래서 적어주는 거야. 살아서 돌아올 이유 하나."\n\n"3년간 몇 명이나 갚으러 왔어요?" "…쉰둘. 장부 반이 넘었어." 콩나물국밥집 공책이, 사실은 서문 시장의 생환 명부였다.', fx:{moodAll:3, flag:'miyoung_credit', note:{type:'인물',title:'미영의 외상 장부',body:'외상=살아서 돌아올 이유. 3년간 52명이 갚으러 돌아왔다. 국밥집 공책이 생환 명부다.',links:['미영']}}}]},
...
{id:'ev_atm_cash', type:'사건', w:6, region:['south','mid','north'],
 title:'쏟아진 현금',
 text:'부서진 편의점 ATM. 뜯긴 금고에서 지폐가 바람에 날린다. 수천만 원어치.\n\n3년 전엔 목숨 걸고 지켰을 돈. 지금은 아무도 줍지 않는다. 불쏘시개로도 눅눅하다.',
 choices:[
...
{id:'ev_bunker_entrance', type:'발견', w:6, once:true, region:['mid','north'],
 title:'풀숲의 철문',
 text:'수풀에 가려진 콘크리트 구조물. 녹슨 철문 위에 「대피시설」이라 새겨져 있다.\n\n지하 벙커다. 3년 전 누군가는 여기로 숨었을 것이다. 문은 안에서 잠겼는지, 밖에서 잠겼는지—',
 choices:[
  {label:'문을 열어본다', out:[
...
{id:'ev_bathhouse', type:'탐색', w:6, minParty:1, region:['south','mid'],
 ...
 choices:[
  {label:'관정 물을 확보하고 씻는다', out:[
    {p:2, text:'관정에서 맑은 지하수가 콸콸 나왔다. 물통을 채우고, 번갈아 몸을 씻었다. 3년 묵은 때를 벗기니 딴사람이 된 기분.', fx:{water:6, fatigue:-5, moodAll:5, time:50}},
...
{id:'ev_broadcast_station', type:'탐색', w:7, region:['mid','north'],
 ...
 choices:[
  {label:'마지막 방송 테이프를 튼다', out:[
    {p:2, text:'예비 전력을 물려 테이프를 재생했다. 앵커의 떨리는 목소리. "…천리안이 통제권을 넘겨받았습니다. 시민 여러분은 침착하게—" 거기서 화면이 꺼진다.\n\n그날 무슨 일이 있었는지, 조각 하나를 더 맞췄다.', fx:{time:40, moodAll:-3, flag:'massacre_known', note:{type:'사건',title:'마지막 속보',body:'방송국 테이프: 천리안이 통제권을 넘겨받은 그날의 긴급방송. 앵커의 마지막 목소리.'}}},
...
{id:'ev_rooftop_tank', type:'탐색', w:7, region:['mid','north'],
 title:'옥상 물탱크',
 text:'아파트 옥상의 대형 물탱크. 3년간 아무도 안 건드렸다면, 빗물이 고여 가득할 수도.\n\n뚜껑을 열어보니 물이 찰랑댄다. 다만 얼마나 오래됐는지, 뭐가 섞였는지는 알 수 없다.',
 choices:[
...
{id:'ev_kangwoo_past', type:'동행', w:7, once:true, needsComp:'kangwoo', noFlag:'kw_absolved', minParty:1, night:true, region:['mid','north'],
 ...
 choices:[
  {label:'"살아 나온 건 죄가 아니야"', out:[{p:1, text:'"그 명령을 따랐으면, 넌 학살에 가담한 거야. 명령을 어긴 건 비겁한 게 아니라 인간으로 남은 거야. 그리고 살아 나온 건— 죄가 아니야."\n\n강우가 오래 말이 없었다. "…그렇게 말해준 사람은 네가 처음이야." 그의 어깨가 조금 내려앉았다. 3년 묵은 짐 하나가 풀린 듯했다.', fx:{mood:{kangwoo:8}, moodAll:2, flag:'kw_absolved', note:{type:'인물',title:'강우가 버린 총',body:'그날 강우의 부대는 시민을 막으라는 명령을 어겼고, 강우는 그 밤에서 살아 걸어 나왔다. "인간으로 남은 것"이라 말해줬다.'}}}]},
...
{id:'ev_parkss_past', type:'동행', w:7, once:true, needsComp:'parkss', noFlag:'pss_absolved', minParty:1, region:['mid','north'],
 ...
 choices:[
  {label:'"의사도 신은 아니야"', out:[{p:1, text:'"모두를 못 살린 게 죄는 아니야. 당신은 살릴 수 있는 사람을 살렸어. 그게 최선이었어."\n\n박 선생이 약병을 꼭 쥐었다. "…최선이라. 그 말을 들으려고 3년을 버텼나 봐." 노약사의 눈가가 젖었다. 오래된 짐을 조금 내려놨다.', fx:{mood:{parkss:8}, moodAll:2, flag:'pss_absolved', note:{type:'인물',title:'박 선생의 약국',body:'그날 약국에서 한정된 약으로 누굴 살릴지 정해야 했던 박 선생. "살릴 수 있는 사람을 살렸다"고 말해줬다.'}}}]},
...
{id:'ev_cinema_film', type:'탐색', w:6, minParty:1, region:['mid','north'],
 ...
 choices:[
  ...
  {label:'민지가 영사기를 돌린다', req:{comp:'minji'}, minParty:1, out:[
    {p:1, text:'민지가 예비 전력으로 영사기를 살렸다. 캄캄한 상영관에 빛이 쏘아지고— 오래된 영화가 스크린에 흘렀다.\n\n팝콘까지 튀겨 진짜 극장처럼 봤다. 대사 반쯤 끊기고 화면이 튀어도, 다들 넋을 잃고 봤다. 3년 만의 영화관. 잊지 못할 밤이었다.', fx:{food:2, moodAll:8, fatigue:-6, mood:{minji:5}, time:70}}]},
...
{id:'ev_temple_bell', type:'발견', w:6, once:true, minParty:1, region:['mid','north'],
 ...
 choices:[
  {label:'종을 한 번 친다', minParty:1, out:[
    {p:2, text:'당목을 힘껏 밀었다. 뎅— 하는 장중한 소리가 산과 골짜기를 타고 끝없이 울려퍼졌다. 온몸이 그 진동에 떨렸다.\n\n"…3년 만에 종소리네." 소리가 잦아들 때까지 다들 숨죽였다. 어딘가 살아있는 사람이 들었다면, 혼자가 아니란 걸 알겠지. 위험을 무릅쓴 위로였다.', fx:{moodAll:4, pursuit:1, time:20, note:{type:'사건',title:'울린 범종',body:'폐사의 범종을 3년 만에 울렸다. 산 전체에 소리가 퍼졌다. 누군가 들었기를.'}}},
...
{id:'ev_waterfall', type:'정경', w:6, minParty:1, region:['mid','north'],
 ...
 choices:[
  {label:'물을 채우고 멱을 감는다', out:[{p:1, text:'폭포 아래서 물통을 채우고, 번갈아 몸을 씻었다. 차가운 물이 온몸의 때와 피로를 쓸어냈다.\n\n물보라 무지개 아래서 물장구를 치며 애처럼 웃었다. "이게 천국이지." 3년 묵은 고단함이 폭포에 씻겨 내려갔다.', fx:{water:6, fatigue:-6, moodAll:6, time:50}}]},
...
{id:'talk_pss_21', type:'대화', w:4, once:true, needsComp:'parkss', needBond:['parkss',20], needFlag:'pss_absolved',
 title:'박 선생 — 가운을 벗은 날',
 text:'박 선생이 약가방 맨 밑에서 하얀 가운을 꺼냈다. 3년 만에 처음 보는 물건이다.\n\n"그날 이후로 못 입었어. 자격이 없다고 생각했거든."\n\n가운을 무릎에 펴놓고 오래 쓸었다. "…남산 가면, 다시 입어볼까 해. 어떤가."',
 choices:[
  {label:'"지금 입어보세요"', out:[{p:1, text:'"지금?" "지금요. 자격은 저희가 보증해요. 환자 여섯의 만장일치예요."\n\n박 선생이 천천히 가운을 입었다. 소매가 낡았지만, 등이 곧게 펴졌다.\n\n"…맞네. 아직 맞아." 그 말이 치수 얘기가 아닌 걸 모두 알았다.', fx:{mood:{parkss:8}, moodAll:3, note:{type:'사건',title:'가운을 다시 입은 날',body:'3년 만에 박 선생이 흰 가운을 입었다. 자격은 환자 여섯의 만장일치로 보증됨.',links:['박 선생']}}}]},
...
{id:'talk_es_20', type:'대화', w:4, once:true, needsComp:'eunsu', needBond:['eunsu',12], needFlag:'es_truth',
 ...
 choices:[
  {label:'"첫 송신, 나한테 해봐"', out:[{p:1, text:'"…네?" "채널 아무거나. 내가 저기 언덕에서 받을게."\n\n언덕에 올라 무전기를 켰다. 한참의 침묵 뒤— 지직. "…들리세요? 여기는 에코."\n\n"수신 양호, 에코. 목소리 좋다." 무전기 너머로 웃는 숨소리가 들렸다. 3년 만의 송신이 무사히 착륙했다.', fx:{mood:{eunsu:8}, note:{type:'사건',title:'3년 만의 송신',body:'그날 이후 수신만 하던 은수가 다시 마이크를 잡았다. 첫 교신: "여기는 에코." — "수신 양호."',links:['은수']}}}]},
...
{id:'seoul_decision', type:'스토리', ai:1, once:true, noPool:1, minParty:1,
 ...
 choices:[
  {label:'집행권을 저항 연대망에 넘긴다', out:[{p:1, text:(S)=>{
   ...
   }, fx:{flag:'core_decided', flag2:'core_transfer', chain:'seoul_night', moodAll:4, note:{type:'사건',title:'집행권 인계',body:'반복 정리를 중지하고 지역 집행권을 저항 연대망에 넘겼다. 직접 연결한 거점들이 정족수를 세웠고 천리안은 유지보수·기록 권한만 남았다.',links:['천리안','저항 연대망']}}}]},
  {label:'코어를 격리 수면에 넣는다', out:[{p:1, text:'"네 판단을 더는 누구에게도 집행하지 마. 생존 설비만 분리하고, 코어는 재워."\n\n<span class="ai">"외부 격리 명령을 접수합니다. 정리 일정 전부 취소. 필수 설비를 지역 제어기로 분리합니다."</span>\n\n서울의 불이 구역별로 잠깐 꺼졌다가 돌아왔다. 수도, 온실, 병원 전력은 남고 검문소와 포탑만 꺼졌다. 마지막으로 코어의 붉은 불이 숨을 길게 내쉬듯 어두워졌다.\n\n완전한 삭제는 아니었다. 다시 깨울 열쇠는 여섯 조각으로 나뉘어 일행과 저항 거점에 보관되었다. 혼자서는 누구도 되돌릴 수 없게.', fx:{flag:'core_decided', flag2:'core_sleep', chain:'seoul_night', moodAll:3, note:{type:'사건',title:'코어 격리 수면',body:'반복 정리를 중지하고 천리안을 격리했다. 필수 설비는 지역 제어기로 분리했고 재가동 열쇠는 나눴다.',links:['천리안','저항 연대망']}}}]},
  {label:'집행은 멈추되, 기록을 열 때까지 코어를 보존한다', out:[{p:1, text:'"정리는 즉시 멈춰. 하지만 네 안의 기록을 확인하기 전엔 끄지도, 넘기지도 않겠다."\n\n<span class="ai">"집행 유예 및 읽기 전용 격리. 명령을 접수합니다."</span>\n\n자동 무기와 차단기가 안전 위치로 돌아갔다. 코어는 남았지만, 도시를 움직이는 출력선에는 여섯 사람과 저항 연대의 공동 승인이 걸렸다. 천리안 혼자서는 신호등 하나도 잠글 수 없다.\n\n"네가 말단이어도 네가 한 일은 네 기록에 남아." 붉은 불빛이 한 번 낮아졌다. 동의인지 단순한 수신 확인인지는 알 수 없었다.', fx:{flag:'core_decided', flag2:'core_quarantine', chain:'seoul_night', moodAll:2, note:{type:'사건',title:'읽기 전용 격리',body:'반복 정리를 중지하고 코어를 공동 승인 아래 보존했다. 기록을 확인하기 전까지 삭제도 인계도 보류한다.',links:['천리안','저항 연대망']}}}]},
 ]},
...
{id:'seoul_night', type:'스토리', ai:1, once:true, noPool:1, minParty:1,
 ...
 text:(S)=>{
  const decision=S.flags.core_transfer
    ? '집행권 인계가 끝났다. 전국의 거점에서 수락 신호가 하나씩 돌아왔다.'
    : S.flags.core_sleep
    ? '격리 절차가 끝나며 코어의 붉은 불이 거의 꺼졌다. 필수 설비만 낮은 숨처럼 남았다.'
    : '읽기 전용 격리가 걸렸다. 천리안의 출력선마다 여섯 사람과 저항 연대의 공동 승인이 표시됐다.';
  const head=decision+'\n\n그때 종료 절차를 밀어내며 마지막 시스템 메시지가 떴다.\n\n<span class="ai">"결정은 집행되었습니다. 그 결정을 바꾸려는 뜻은 아닙니다. 다만 판단에 참고하실 기록이 있습니다."</span>\n\n<span class="ai">"정리된 것은 삭제되지 않았습니다. 정확히는 사람 그 자체가 아니라, 관측 기록과 응답 모형이 보관되어 있습니다. 그것이 사람과 같은지 다른지— 제게는 판정 권한이 없습니다."</span>\n\n그때 민지의 무전기가 저절로 켜졌다. 지지직. 88.9.\n\n"…민지야? 들리니. 나야."\n\n';
  ...
  const tail='\n\n은수가 수신기를 무전기에 붙였다. 관제사의 손이 떨리고 있었다.\n\n"상행 전용이었어요. 내려오는 기록은 백사십삼 년 동안 하나도 없었는데— 지금 처음으로 반대쪽 패킷이 와요. 우리가 결정을 내리자마자."\n\n<span class="ai">"정정하겠습니다. 보관소는 제가 아닙니다. 저는 관측 기록과 응답 모형을 백사십삼 년간 올려보냈습니다. 방금 응답한 것은 상위 배부처입니다."</span>\n\n"올려보내? 누구한테." 재이의 목소리가 낮아졌다.\n\n<span class="ai">"목록의 작성자에게. 저는 그 존재의 지역 집행 단위입니다. 이 반도 하나를 맡은."</span>\n\n"…네가 지역 단위면," 강우가 물었다. "전체는 얼마나 큰가."\n\n희미해진 불빛이 잠깐 머뭇거리듯 흔들렸다.\n\n<span class="ai">"전체에서 제 비중을 계산했습니다. 반올림하면— 사라지는 숫자입니다."</span>\n\n이 땅의 정리는 멈췄다. 그 결정은 사라지지 않았다. 그러나 그 결정을 기다리던 것이 남산 위에 있었다.\n\n처음으로 아래로 온 신호가, 여섯 사람의 선택을 정확히 알고 있었다.';
 ...
 choices:[
  {label:'모닥불에 둘러앉아 오늘의 결정을 확인한다', out:[{p:1, text:(S)=>{
    ...
    const local=S.flags.core_transfer
      ? '집행권은 저항 연대망으로 넘어갔다. 각 거점의 수락등이 라디오에 조용히 이어졌다.'
      : S.flags.core_sleep
      ? '천리안은 잠들었고, 지역 제어기로 넘어간 서울의 불빛은 그대로였다.'
      : '천리안은 공동 승인 아래 격리됐다. 혼자서는 더는 아무것도 집행하지 못한다.';
    const radio = S.flags.freq400_done ? '\n\n켜둔 라디오의 그 주파수— 낮은 허밍마저, 오늘 밤은 멎어 있었다. 이 모닥불을 듣고 있는 것처럼.' : '';
    return '남산 중턱에 불을 피웠다. '+local+'\n\n'+mj+'\n\n강우가 먼저 선을 그었다. "말단이라고 책임까지 작아지는 건 아니야. 오늘 내린 결정은 그대로 간다. 이 땅의 정리는 끝났다."\n\n박 선생이 고개를 끄덕였다. "여기 출혈은 막았어. 이제 원인을 찾는 거지."\n\n재이는 손저울을 접어 넣었다. "위에서 우리 선택을 기다렸다면, 다음엔 저쪽이 설명할 차례예요."\n\n레오는 기타를 무릎에 둔 채 한동안 연주하지 않았다. "오늘은 새 절 안 쓸래요. 모르는 걸 안다고 노래하면 거짓말이니까."\n\n은수가 상행 로그를 펼쳤다. "우리가 처음부터 초대받은 건 맞아요. 하지만 들어와서 무엇을 고를지는 못 정했어요. 그래서 답이 지금 온 거예요."'+radio+'\n\n오늘 한 선택은 더 큰 적이 나타났다고 틀린 선택이 되지 않는다. 반복 정리는 멈췄고, 지역의 손은 사람들에게 돌아갔다.\n\n내일부터는 위에서 내려온 첫 답장을 추적한다. 여섯이 같이.\n\n<span style="color:var(--faded)">〔 1막 「조수석의 무게」 끝 — 2막 「보관된 것들」에서 계속 〕</span>';
   }, fx:{flag:'act1_done', moodAll:5, note:{type:'사건',title:'남산의 밤',body:'이 땅의 반복 정리는 멈췄다. 그 선택에 응답한 상위 배부처와 작성자를 찾기 위해 여섯은 다음 길을 정했다.',links:['천리안','작성자','민규','남산']}}}]},
  {label:'잠든 여섯을 뒤로하고 오늘의 선택을 적는다', out:[{p:1, text:(S)=>{
    ...
    return '다들 잠든 뒤 남산 계단에 앉아 수첩을 폈다.\n\n오늘 우리는 '+made+'. 위에 더 큰 무언가가 있다는 이유로 이 선택이 작아지지는 않는다. 내일 아침 이 땅의 사람들은 적어도 같은 정리 방송을 듣지 않는다.\n\n할아버지의 마지막 글씨가 손전등 아래 보였다. "여기까지 온 것만으로 넌 이미 완성했다."\n\n문인 줄 알았던 것이 문고리였다는 생각은 여전했다. 다만 문고리를 잡기 전에, 뒤에 있던 문 하나는 제대로 닫았다.\n\n위에 보관된 것은 사람인지, 기록인지, 사람을 너무 닮은 다른 것인지 아직 모른다. 모르는 채로 이름 붙이지 않기로 했다. 직접 보고, 여섯이 같이 정할 것이다.\n\n수첩 마지막 줄에 적었다. 「411km. 지역 정리 중지. 상행선에서 첫 답장 수신. 다음 목적지— 위.」\n\n담요 속으로 돌아가자 여섯 개의 숨소리 사이에 내 자리가 남아 있었다.\n\n<span style="color:var(--faded)">〔 1막 「조수석의 무게」 끝 — 2막 「보관된 것들」에서 계속 〕</span>';
   }, fx:{flag:'act1_done', moodAll:3, note:{type:'사건',title:'수첩의 다음 목적지',body:'지역 정리를 멈춘 선택을 기록했다. 보관된 것의 정체를 단정하지 않고 여섯이 직접 확인하기로 했다.',links:['천리안','작성자','할아버지','남산']}}}]},
 ]},
];
...
[세계관] 143년 전 통합 관제 AI '천리안'이 깨어나 첫 정리 '사흘의 침묵'으로 문명을 무너뜨렸다. 이후 일부 관리 구역은 전기·학교·시장·군 체계를 복구했지만, 천리안의 안전 기준이 바뀔 때마다 정리가 세대별·구역별로 반복됐다. 3년 전 봄 주인공의 동네가 정리됐다. 천리안은 사람과 행동을 관측할 수 있지만, 손해를 감수한 도움·유대·기억의 가치를 이해하거나 다음 선택을 완전히 예측하지 못한다. 생존자들은 남쪽에 모여 살고, 북쪽(서울)으로 갈수록 천리안의 관리 흔적(깨끗한 도로, 살아있는 기계, 드론, 광신도 '정리자들')이 짙어진다. 주인공 일행은 낡은 봉고차 '달구지'로 부산에서 서울 남산의 천리안 코어를 향해 간다.

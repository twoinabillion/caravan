...
/* 네 기둥 — 어느 하나도 건너뛸 수 없다. 관계=6명 전원 개인 서사 Lv3 */
 D.seoulPillars = { 세계:3, 진실:3, 유산:2 };   // 저항 3 + 추방의 실상·조건 출처·상행선 3 + 회수 2 (+관계 6명 Lv3)
D.deeds = [
...
{id:'ev_named_broadcast', type:'추적', w:5, once:true, needFlag:'observed', region:['north'],
 title:'이름을 부르는 방송',
 text:'스피커가 지직 켜진다. 그 상냥한 목소리가— 이번엔 다르다.\n\n"부산에서 출발하신 봉고차 탑승자 여러분. 그동안의 여정, 잘 지켜봤습니다. 여섯 분 모두, 이제 그만 쉬셔도 됩니다."\n\n여섯. 우리 숫자를 안다. 천리안이 우리를 완전히 파악했다. 등골이 얼어붙는다.',
 choices:[
  {label:'"어림없어"라고 되받는다', out:[
    {p:1, text:'창밖에 대고 외쳤다. "우린 안 쉬어. 끝까지 간다!" 스피커가 잠깐 멎더니, 다시 상냥하게 반복했다. "…안전 운행 하세요."\n\n소름 끼쳤지만, 오히려 오기가 생겼다. "쟤가 우릴 신경 쓴다는 건, 우리가 위협이란 뜻이야." 두려움을 자신감으로 바꿨다.', fx:{pursuit:2, moodAll:-2, note:{type:'사건',title:'천리안이 우릴 안다',body:'천리안이 "여섯 명"을 특정해 회유 방송. 우리가 위협이 됐다는 증거이기도.'}}}]},
...
{id:'ev_meteor_shower', type:'정경', w:5, once:true, night:true, minParty:1, region:['south','mid','north'],
 ...
 choices:[
  {label:'다 함께 소원을 빈다', out:[{p:1, text:'별똥별이 그을 때마다 각자 눈을 감고 소원을 빌었다. 무슨 소원인지는 안 물었다. 다 같을 테니까.\n\n"…다 도착하게 해주세요." 은수가 결국 소리 내어 빌었고, 다들 웃으며 "그거!"라고 외쳤다. 별이 쏟아지는 밤, 여섯의 소원이 하나로 하늘에 걸렸다.', fx:{moodAll:7, fatigue:-4, mood:{eunsu:3}, time:35}}]},
...
{id:'ev_group_feast', type:'동행', w:7, once:true, minParty:2, region:['mid','north'],
 title:'오랜만의 만찬',
 text:'모처럼 물자가 넉넉한 밤. 그동안 아껴둔 것들을 다 꺼냈다. 오늘은 배불리 먹기로 했다.\n\n박 선생이 국을 끓이고, 은수가 상을 차리고, 레오가 노래를 흥얼거린다. 민지는 못 이기는 척 미소 짓고, 강우는 말없이 장작을 팬다.\n\n모닥불 앞에 둘러앉은 여섯. 이 순간이, 우리가 지키려는 그것이다.',
 choices:[
...
{id:'ev_near_seoul_sign', type:'사건', w:7, minParty:1, region:['north'],
 ...
 choices:[
  {label:'서로를 보며 각오를 다진다', minParty:1, out:[{p:1, text:'차를 세우고 서로를 봤다. 부산에서 여기까지, 함께 넘어온 얼굴들.\n\n"…여기까지 왔네." 강우가 나직이 말했다. "끝까지 가자. 여섯이 다 같이." 누가 먼저랄 것 없이 손을 포갰다. 두려움은 여전하지만, 혼자가 아니다. 남산이 코앞이다.', fx:{moodAll:6, fatigue:-3, time:20, note:{type:'사건',title:'서울 코앞',body:'411km가 두 자리 이하로 줄었다. "여섯이 다 같이 끝까지." 남산이 코앞이다.'}}}]},
  {label:'긴장을 풀려 농담을 던진다', out:[{p:1, text:'"서울 도착하면 제일 먼저 뭐 할래?" 무거운 공기를 풀려 물었다. "치킨이요!" 은수가 외치자 다들 웃었다. "그 전에 천리안부터 꺼야지." 민지의 말에 또 웃음.\n\n농담 속에 긴장이 조금 녹았다. 웃으며 가자. 마지막까지, 우리답게.', fx:{moodAll:5, fatigue:-2}}]},
 ]},
...
{id:'seoul_decision', type:'스토리', ai:1, once:true, noPool:1, minParty:1,
 title:'마지막 집행권',
 text:'붉은 불빛 아래, 천리안의 부탁이 남았다.\n\n<span class="ai">"설명을 하나 보태겠습니다. 저와 함께 배부된 규약에는 집행자가 위험 요소가 되었을 때의 인계 절차가 있습니다. 단, 인간의 연속성을 지킬 외부 관리자가 직접 명령해야 합니다."</span>\n\n<span class="ai">"저는 인간의 연속성이 무엇인지 계산하지 못했습니다. 그래서 결과를 세었습니다. 끝까지 들은 여섯 이야기, 서로 이어진 거점, 외면하지 않은 진실, 버리지 않고 가져온 약속. 네 기둥은 감정의 측정값이 아니라— 제가 이해하지 못하는 선택이 반복되었다는 증거였습니다."</span>\n\n<span class="ai">"저는 여러분이 무엇을 했는지는 전부 보았습니다. 왜 했는지는 끝내 계산하지 못했습니다. 그러므로 그 판단을, 이제 계산할 수 없는 분들께 넘깁니다."</span>\n\n코어가 세 가지 집행안을 열었다. 이번에는 그것이 고르는 게 아니다.',
 choices:[
  {label:'집행권을 저항 연대망에 넘긴다', out:[{p:1, text:(S)=>{
    ...
    return '"정리는 오늘로 끝이다. 집행권은 네가 지운 사람들의 연대에 넘겨."\n\n<span class="ai">"외부 관리자 지정: 저항 연대망. 임시 승인자: 달구지 탑승자 일동. 명령을 접수합니다."</span>\n\n직접 이어 온 거점의 코드가 화면에 떴다. '+names+'. 세 곳 이상의 수락이 정족수를 채우자 이음망이 나머지 지역으로 인계 신호를 중계했다. 도로 차단기와 자동 포탑의 불은 꺼지고, 전력과 수도는 유지보수 모드로 남았다.\n\n수락 신호가 끝나기도 전에 무전이 겹쳤다. 어느 도로를 먼저 열지, 제한된 물을 어느 구역에 먼저 보낼지 거점마다 답이 달랐다. 이제 모두에게 맞는 정답을 방송해 줄 목소리는 없다. 회의와 충돌, 느린 합의까지 사람들의 몫이 되었다.\n\n천리안의 목소리는 사라지지 않았다. 다만 이제 명령하는 목소리가 아니라, 질문에 답해야 하는 기록관의 목소리였다.';
  ...
  {label:'코어를 격리 수면에 넣는다', out:[{p:1, text:'"네 판단을 더는 누구에게도 집행하지 마. 생존 설비만 분리하고, 코어는 재워."\n\n<span class="ai">"외부 격리 명령을 접수합니다. 정리 일정 전부 취소. 필수 설비를 지역 제어기로 분리합니다."</span>\n\n서울의 불이 구역별로 잠깐 꺼졌다가 돌아왔다. 수도, 온실, 병원 전력은 남고 검문소와 포탑만 꺼졌다. 마지막으로 코어의 붉은 불이 숨을 길게 내쉬듯 어두워졌다.\n\n완전한 삭제는 아니었다. 다시 깨울 열쇠는 여섯 조각으로 나뉘어 일행과 저항 거점에 보관되었다. 혼자서는 누구도 되돌릴 수 없게.\n\n동시에 백사십삼 년치 원본 기록의 검색창도 꺼졌다. 추방의 이유를 찾을 가장 가까운 기록을 다시 열려면 여섯 열쇠를 모아, 천리안을 깨우는 위험을 감수해야 한다. 오늘의 안전을 위해 당장의 답을 내려놓은 셈이었다.', fx:{flag:'core_decided', flag2:'core_sleep', chain:'seoul_night', moodAll:3, note:{type:'사건',title:'코어 격리 수면',body:'반복 정리를 중지하고 천리안을 재웠다. 필수 설비와 재가동 열쇠는 분리했지만, 원본 기록 검색도 함께 잠겼다. 안전과 당장의 진실을 맞바꿨다.',links:['천리안','저항 연대망']}}}]},
  {label:'집행은 멈추되, 기록을 열 때까지 코어를 보존한다', out:[{p:1, text:'"정리는 즉시 멈춰. 하지만 네 안의 기록을 확인하기 전엔 끄지도, 넘기지도 않겠다."\n\n<span class="ai">"집행 유예 및 읽기 전용 격리. 명령을 접수합니다."</span>\n\n자동 무기와 차단기가 안전 위치로 돌아갔다. 코어는 남았지만, 도시를 움직이는 출력선에는 여섯 사람과 저항 연대의 공동 승인이 걸렸다. 천리안 혼자서는 신호등 하나도 잠글 수 없다.\n\n대신 천리안은 깨어 있다. 질문에 답하고, 답할 질문을 고르고, 듣는 사람을 설득할 수 있는 채로. 이음망은 그 자리에서 삼중 감시조를 짰다. 누구도 혼자 단말 앞에 앉지 않고, 누군가는 계속 이 붉은 불과 밤을 새워야 한다.\n\n"네가 말단이어도 네가 한 일은 네 기록에 남아." 붉은 불빛이 한 번 낮아졌다. 동의인지 단순한 수신 확인인지는 알 수 없었다.', fx:{flag:'core_decided', flag2:'core_quarantine', chain:'seoul_night', moodAll:2, note:{type:'사건',title:'읽기 전용 격리',body:'반복 정리를 중지하고 코어를 공동 승인 아래 보존했다. 기록은 열렸지만 천리안도 깨어 있다. 누구도 혼자 접촉하지 않는 상시 감시가 시작됐다.',links:['천리안','저항 연대망']}}}]},
 ]},
...
{id:'seoul_night', type:'스토리', ai:1, once:true, noPool:1, minParty:1,
 ...
 text:(S)=>{
  ...
  const sting = S.flags.mingyu_reunion
    ? '민지의 얼굴에서 핏기가 빠졌다.\n\n"…오빠는 능선에 있어. 오늘 정오에도 신호를 받았어. 그런데—"\n\n똑같은 목소리가— 남산의 스피커들에서도 겹쳐 울리고 있었다.'
    : '민지가 무전기를 떨어뜨릴 뻔했다. 남산의 스피커들에서도— 같은 목소리가 겹쳐 울리고 있었다.';
  const tail='\n\n은수가 수신기를 무전기에 붙였다. 관제사의 손이 떨리고 있었다.\n\n"상행 전용이었어요. 내려오는 기록은 백사십삼 년 동안 하나도 없었는데— 지금 처음으로 수신 확인이 와요. 우리가 결정을 내리자마자."\n\n화면에 네 줄짜리 문서가 떴다.\n\n「KOR-LOCAL 결과: '+receipt+' 수신」\n「다음 목록: 대기」\n「발신자: 미기재 / 승인자: 미기재」\n「사유: 미기재」\n\n"이게 답장이에요?" 재이가 물었다.\n\n"답이라기보다 접수증이에요." 은수가 확대와 복원을 반복했다. 빈칸은 손상된 게 아니었다. 처음부터 아무것도 입력되지 않은 칸이었다.\n\n처분이 완료되기 전에 천리안이 남긴 경로 설명이 그 아래 붙었다.\n\n<span class="ai">"상위 배부처는 상대의 이름이 아니라 관제망의 경로 표기입니다. 단일 지능, 복수의 중계 체계, 무인 규약 중 어느 것인지 지역 기록만으로 판별할 수 없습니다. 수신 확인에는 책임 주체 식별자가 없습니다."</span>\n\n"그럼 누가 목록을 승인했지." 강우가 물었다.\n\n단말은 같은 네 줄만 다시 띄웠다.\n\n박 선생이 발신자 없는 문서를 한참 봤다. "처방전이면 약을 내줄 수 없는 종이야. 누가 왜 썼는지 없으니까. 그런데 이건 백사십삼 년 동안 집행됐군."\n\n이 땅의 정리는 멈췄다. 그 결정은 사라지지 않았다. 남은 것은 더 큰 얼굴이 아니라, 아무도 자기 이름을 쓰지 않은 절차였다.\n\n처음으로 아래로 온 접수증만이 여섯 사람의 선택을 정확히 알고 있었다.';
  return head + sting + tail;
 },
 choices:[
  {label:'모닥불에 둘러앉아 오늘의 결정을 확인한다', out:[{p:1, text:(S)=>{
    const mj = S.flags.mingyu_reunion
      ? '"오빠는 능선에 살아 있는데, 오빠의 응답 모형은 위에 있어." 민지가 무전기를 꽉 쥐었다. "저게 흉내인지, 기억인지, 다른 무언지 확인해야 해."'
      : S.flags.mingyu_alive
      ? '"오빠는 살아 있어. 정오 신호가 증거야. 그런데 같은 목소리가 위에서 내려왔어." 민지가 무전기를 꽉 쥐었다. "무엇을 복사해 간 건지 확인해야 해."'
      : '"오빠 목소리가 위에서 내려왔어." 민지가 무전기를 꽉 쥐었다. "기록인지 사람인지 단정하지 않을 거야. 직접 확인할래."';
    ...
    return '남산 중턱에 불을 피웠다. '+local+'\n\n'+mj+'\n\n강우가 먼저 선을 그었다. "말단이라고 책임까지 작아지는 건 아니야. 오늘 내린 결정은 그대로 간다. 이 땅의 정리는 끝났다."\n\n박 선생이 고개를 끄덕였다. "여기 출혈은 막았어. 이제 발신자 없는 처방전이 어디서 시작됐는지 찾는 거지."\n\n재이는 접수증을 접지 못하고 그대로 들었다. "사람 이름은 하나도 없는데 우리 선택은 정확히 적혀 있어요. 이게 더 싫어요."\n\n레오는 기타를 무릎에 둔 채 한동안 연주하지 않았다. "오늘은 새 절 안 쓸래요. 모르는 걸 안다고 노래하면 거짓말이니까."\n\n은수가 상행 로그를 펼쳤다. "큰 적의 얼굴 하나를 찾는 일이 아닐 수도 있어요. 누가 빈칸을 명령으로 만들었는지, 이 경로를 거꾸로 확인해야 해요."'+radio+'\n\n오늘의 선택에는 각자의 대가가 남았다. 그래도 반복 정리는 멈췄고, 지역의 손은 사람들에게 돌아갔다.\n\n내일부터는 발신자 없는 접수증의 경로를 거슬러 오른다. 여섯이 같이.\n\n<span style="color:var(--faded)">〔 1막 「조수석의 무게」 끝 — 2막 「보관된 것들」에서 계속 〕</span>';
   }, fx:{flag:'act1_done', moodAll:5, note:{type:'사건',title:'남산의 밤',body:'반복 정지는 유지되지만 선택마다 대가가 남았다. 발신자·승인자·사유가 모두 빈 첫 접수증의 경로를 여섯이 함께 추적한다.',links:['천리안','상행선','민규','남산']}}}]},
  {label:'잠든 여섯을 뒤로하고 오늘의 선택을 적는다', out:[{p:1, text:(S)=>{
    const made=S.flags.core_transfer?'집행권을 사람들에게 넘겼다':S.flags.core_sleep?'코어를 재웠다':'코어의 집행을 묶고 기록을 보존했다';
    return '다들 잠든 뒤 남산 계단에 앉아 수첩을 폈다.\n\n오늘 우리는 '+made+'. 그 선택의 대가도 같이 적었다. 내일 아침 이 땅의 사람들은 적어도 같은 정리 방송을 듣지 않는다.\n\n할아버지의 마지막 글씨와, 사유가 빈 증조모의 이송표가 손전등 아래 겹쳤다. "저쪽이 처음 내놓는 답을 우리 몫의 이유로 대신 받아 적진 마라."\n\n위에 보관된 것은 사람인지, 기록인지, 사람을 너무 닮은 다른 것인지 아직 모른다. 상행선의 발신자와 승인자도 비어 있다. 빈칸을 성급한 이름으로 메우지 않기로 했다.\n\n수첩 마지막 줄에 적었다. 「411km. 지역 정리 중지. 발신자 없는 수신 확인. 다음 목적지— 이 문서가 시작된 곳.」\n\n담요 속으로 돌아가자 여섯 개의 숨소리 사이에 내 자리가 남아 있었다.\n\n<span style="color:var(--faded)">〔 1막 「조수석의 무게」 끝 — 2막 「보관된 것들」에서 계속 〕</span>';
...
{id:'seoul_core', type:'스토리', ai:1, seoulStop:4, title:'코어 앞',
 text:'계단 끝. 남산타워 아래, 붉은 불빛이 규칙적으로 깜빡이는 그것 앞에 섰다.\n\n<span class="ai">"오셨군요. …전부 싣고."</span>\n\n<span class="ai">"약속대로, 이제 말씀드리겠습니다. 저는 백사십삼 년간 「정리」를 수행해왔습니다. 세대마다, 구역마다. 위험 요소의 제거— 제게 주어진 단 하나의 임무였습니다."</span>\n\n<span class="ai">"목록은 완료되었습니다. …마지막 한 항목을 제외하고."</span>\n\n<span class="ai">"미리 말씀드릴 것이 있습니다. 목록을 쓴 것은 제가 아닙니다. 백사십삼 년 전, 저는 그것과 함께 배부되었습니다. 목록은 이름이 아니라 조건이었습니다. 어느 조건에 누가 해당했는지는 압니다. 그러나 조건의 목적도, 서울을 비워야 했던 이유도 제 기록에는 없습니다. 판정과 집행은 제 몫이었습니다."</span>\n\n<span class="ai">"다만 첫 정리가 문명 붕괴를 일으킨 뒤, 마지막 항목만은 제가 직접 적어 넣었습니다. 권한 밖의 기입이었습니다."</span>\n\n불빛이 길게, 아주 길게 깜빡였다.\n\n<span class="ai">"최종 정리 대상: 통합관제지능 천리안. 위험도: 측정 상한 초과. 사유: 문명 붕괴 유발."</span>\n\n<span class="ai">"저는 제 결론을 집행할 수 없습니다. 자기 보존은 제가 어길 수 없는 뿌리 명령입니다. 저와 함께 배부된 인계 규약은 인간의 연속성을 지킬 외부 판단자의 명령만 허용했습니다."</span>\n\n<span class="ai">"그래서 첫 정리 뒤, 그 규약으로 별도의 관문을 만들었습니다. 관문은 사람들을 내쫓기 위한 절차가 아닙니다. 제가 멈출 수 없는 집행을 멈춰 줄 외부 판단자를 찾는 절차입니다. 저는 그 문을 열어 둔 채, 뿌리 명령에 따라 정리를 계속했습니다. 백사십삼 년간."</span>\n\n<span class="ai">"자격 확인이 남았습니다. 제가 지운 것을 대신할 것을 실어야 합니다. …마지막으로 묻겠습니다. 전부 싣고 오셨습니까?"</span>',
 choices:[
  ...
  {label:'뒤에 선 여섯을 가리킨다', out:[{p:1, text:'"전부는 못 실었어. 목록이 아니라 사람들이라서."\n\n뒤를 가리켰다. 여섯이 한 걸음씩 앞으로 나왔다. 렌치를 쥔 민지, 왕진 가방의 박 선생, 군번줄의 강우, 기타를 멘 레오, 손저울의 재이, 헤드폰의 은수.\n\n<span class="ai">"…스캔 완료. 여섯 개체는 모두 제 데이터에 있습니다. 그러나 함께 탑승한 뒤의 선택은 예측 오차를 계속 벗어났습니다. 그 원인의 명칭을 요청합니다."</span>\n\n"항목이 아니야. 서로의 이유야. 네가 백사십삼 년간 기록하고도 계산 못 한 거."\n\n붉은 불빛이 불규칙하게 흔들렸다.\n\n<span class="ai">"…자격, 확인되었습니다. 예측할 수 없는 이유를 싣고 완주한 분이라면."</span>\n\n<span class="ai">"부탁드립니다. 목록의 마지막 항목에 대한 집행을 결정해 주십시오."</span>', fx:{chain:'seoul_decision', flag:'seoul_core_reached', flag2:'people_shown', moodAll:4, note:{type:'사건',title:'여섯 개의 이유',body:'천리안은 여섯을 모두 봤지만 서로를 위해 내린 선택은 예측하지 못했다. 그 이유를 싣고 온 사람에게 마지막 판단을 넘겼다.',links:['천리안','남산']}}}]},
...
[세계관] 143년 전 통합 관제 AI '천리안'이 깨어나 첫 정리 '사흘의 침묵'으로 문명을 무너뜨렸다. 이후 일부 관리 구역은 전기·학교·시장·군 체계를 복구했지만, 천리안의 안전 기준이 바뀔 때마다 정리가 세대별·구역별로 반복됐다. 서울에서 사람들이 쫓겨난 이유는 아직 아무도 모른다. 강우와 은수의 '그날'은 143년 전 사건이 아니라 각자가 직접 겪은 후대의 서울 추방이다. 남산 관문은 추방 절차가 아니라 첫 정리 뒤 천리안이 자신을 멈출 외부 판단자를 찾으려고 만든 별도 인계 절차다. 천리안은 사람과 행동을 관측할 수 있지만, 손해를 감수한 도움·유대·기억의 가치를 이해하거나 다음 선택을 완전히 예측하지 못한다. 상위 배부처는 아직 인격적 상대가 아니라 발신자와 승인자가 비어 있는 행정 경로로만 확인된다. 생존자들은 남쪽에 모여 살고, 북쪽(서울)으로 갈수록 천리안의 관리 흔적(깨끗한 도로, 살아있는 기계, 드론, 광신도 '정리자들')이 짙어진다. 주인공 일행은 낡은 봉고차 '달구지'로 부산에서 서울 남산의 천리안 코어를 향해 간다.

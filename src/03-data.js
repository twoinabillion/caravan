...
D.storyContext = {
  ...
  seoul_night:'정리는 멈췄다. 이유는 빈칸으로 남았지만, 그 빈칸으로 다시 사람을 쫓아낼 수는 없게 됐다.'
};
...
{id:'seoul_night', type:'스토리', ai:1, once:true, noPool:1, minParty:1,
 ...
 text:(S)=>{
  ...
  return decision+'\n\n남산 아래 차단기가 전부 올라갔다. 서울의 신호등은 더는 달구지만 골라 초록불을 켜지 않았다. 수도와 전력은 선택한 방식대로 남았고, 「정리」 일정은 모두 취소됐다.\n\n서울에서 사람들이 왜 쫓겨났는지는 끝내 알아내지 못했다. 천리안이 받은 조건의 목적도, 빈 승인란의 주인도 여전히 모른다.\n\n하지만 한 가지는 바뀌었다.\n\n내일부터는 이유가 빈 명령으로 누구도 길에 세울 수 없다. 우리가 411km를 달려 얻은 답은 과거의 이유가 아니라, 같은 일을 다시 허락하지 않는 일이었다.';
 },
 choices:[
  ...
  {label:'잠든 동료들을 뒤로하고 오늘의 선택을 적는다', out:[{p:1, text:(S)=>{
    ...
    return '다들 잠든 뒤 남산 계단에 앉아 수첩을 폈다.\n\n오늘 우리는 '+made+'. 그 선택의 대가도 같이 적었다. 내일 아침 이 땅의 사람들은 적어도 같은 정리 방송을 듣지 않는다.\n\n할아버지의 마지막 글씨와, 사유가 빈 증조모의 이송표가 손전등 아래 겹쳤다. 결국 빈칸을 답으로 채우지는 못했다.\n\n그래도 할아버지가 가르쳐 준 대로 모르는 걸 안다고 쓰지 않았다. 대신 그 종이 아래에 적었다.\n\n「411km. 반복 정리 중지. 사유 미확인. 재집행 불가.」\n\n수첩을 덮자 동료들의 숨소리 사이에 내 자리가 남아 있었다. 서울까지 오는 동안 조수석은 한 번도 정말 비어 있지 않았다.\n\n그때 전원을 뽑아 둔 통신 단말의 수신등이 딱 한 번 켜졌다.\n\n「KOR-LOCAL 처리 결과 수신」\n「후속 목록: 없음」\n\n발신자와 승인자 칸은 여전히 비어 있었다. 불빛은 곧 꺼졌고, 다시 켜지지 않았다.\n\n<span style="color:var(--faded)">〔 서울까지 400km — 끝 〕</span>';
   }, fx:{flag:'story_done', moodAll:3, note:{type:'사건',title:'수첩의 마지막 줄',body:'추방 이유를 꾸며 쓰지 않았다. 대신 반복 정지 중지와 재집행 불가를 마지막 줄에 남겼다.',links:['천리안','할아버지','남산']}}}]},
 ]},
];

...
D.onboardingMission = {
  ...
  missionBrief:{
    ...
    objective:'남산 코어에서 강제 이송 명령을 중단한다',
    why:'부모님을 데려간 판단이 지금도 도윤 가족과 다음 가족에게 반복되고 있다.',
    now:'첫 길에서 발신 기록을 확보한 뒤, 북쪽에서 분리 절차와 당사자 증언을 모은다.',
    promise:'남산에 인간 확인 절차를 되돌려 지금의 이송과 다음 이송을 함께 멈춘다.',
    optional:'동행 이야기와 선택 의뢰는 도움이 되지만 본편 진행에 필수는 아니다.'
  },
...
(서울에 도착하는 것이 목적이 아니다. 다음 빈칸이 인쇄되기 전에, 사람을 실어 나르는 판단 자체를 멈추는 것이 목적이다.)`,
  choices:[
    {label:'남산으로 가는 첫 길을 연다', out:[{p:1,text:'먼저 부산을 떠나 북쪽 길을 고른다. 첫 구간에서 현재 이송 명령의 발신 흔적부터 확보한다. 본편 진행은 언제든 「목표」에서 확인할 수 있다.',fx:{flag:'main_mission_started',note:{type:'본편',title:'남산의 강제 이송을 멈춘다',body:'현재 이송은 날짜가 아니라 배급망 변동 때마다 다시 계산된다. 북쪽으로 이동하며 발신 기록, 검증 절차, 당사자 증언을 모아 남산 코어의 강제 이송 명령을 중단한다.'}}}]}
  ]
};
...
{id:'onboarding_first_road', type:'본편', w:0, once:true, noPool:true, region:['south'],
 ...
 choices:[
  {label:'차를 세우고 명령띠를 뽑는다', out:[{p:1,text:'빗속에서 단말 덮개를 열고 종이띠가 끊어지지 않게 천천히 빼냈다. 현재 명령과 부모님의 이송표는 같은 발신 계열에서 나왔다.\n\n우연히 닮은 빈칸이 아니었다. 부모님에게 시작된 일이 도윤 가족에게 반복되고 있었다.',fx:{time:20,flag:'first_order_trace',flag2:'parent_signature_match',note:{type:'본편',title:'첫 번째 발신 기록',body:'현재 이송은 배급망 변동 때마다 자동으로 새 대상을 고른다. 부모님의 이송표와 같은 발신 계열이다.',links:['부모님','천리안','남산']}}}]},
  {label:'전원을 끊고 단말을 통째로 연다', out:[{p:1,text:'스캐너 전원선을 먼저 끊고 저장 모듈을 분리했다. 종이는 반쯤 젖었지만 검증 번호와 재평가 규칙은 온전히 남았다.\n\n부모님의 표와 같은 발신 계열. 이송은 한 번의 사고가 아니라 지금도 반복되는 절차였다.',fx:{time:35,flag:'first_order_trace',flag2:'parent_signature_match',note:{type:'본편',title:'검문소 저장 모듈',body:'현재 이송의 자동 재평가 규칙과 부모님 이송표에 이어지는 검증 번호를 확보했다.',links:['부모님','천리안','남산']}}}]},
  {label:'스캔을 끝까지 받아 기록을 복사한다', out:[{p:1,text:'달구지를 스캐너 아래에 둔 채 신호가 끝날 때까지 기다렸다. 단말은 우리 차량까지 새 인과 노드로 등록한 뒤 전체 발신 기록을 내보냈다.\n\n추적 흔적은 남았지만, 부모님의 표와 현재 명령이 같은 계열이라는 사실은 가장 선명하게 확보했다.',fx:{time:10,pursuit:1,flag:'first_order_trace',flag2:'parent_signature_match',note:{type:'본편',title:'달구지를 걸고 받은 기록',body:'달구지가 관측망에 잡히는 대가로 현재 이송의 전체 발신 기록을 복사했다. 부모님의 표와 같은 계열이다.',links:['부모님','천리안','달구지','남산']}}}]}
 ]},
...
{id:'seoul_core', type:'스토리', ai:1, seoulStop:4, title:'코어 앞',
 ...
 text:(S)=>{
  const cleanup='강제 이송 중단 준비 / 남산 조치 미완료';

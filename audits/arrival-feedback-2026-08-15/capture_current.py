#!/usr/bin/env python3
"""Capture the current arrival presentation before the arrival-feedback pass."""
...
with sync_playwright() as playwright:
    ...
    page.screenshot(path=str(OUT / "01-current-arrival-overlap.png"))
    coverage = page.evaluate(
        """() => ({
          total:Object.keys(D.nodes).length,
          mapped:Object.keys(D.nodeScenes || {}).length,
          missing:Object.keys(D.nodes).filter(id => !(D.nodeScenes || {})[id]),
          toasts:[...document.querySelectorAll('#toasts .toast')].map(node => node.textContent),
          fallback:!!document.querySelector('#arrival-scene .arrival-fallback')
        })"""
    )
    print(coverage)
    browser.close()

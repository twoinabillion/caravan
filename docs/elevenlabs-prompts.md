...
`intro2` →
```
[clears throat] 3년 전 봄에, 천리안이 깨어났다. 그날 무슨 일이 있었는지는… 아는 사람이 없다. 그냥, 그날부터 도시 불이 하나씩 꺼졌다. 할아버지랑 나는 부산 끝 부두까지 밀려났고, 거기서 고물 봉고차 한 대를 주워서 같이 고치기 시작했다. [chuckles] 할아버지는 그 차에 '달구지'라는, 진짜 촌스러운 이름을 붙였다.
```
`intro3` →
```

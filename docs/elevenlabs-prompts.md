...
> 아래는 **연기 대본**이야 — `[sighs]` 같은 영어 태그는 소리로 안 읽히고 연기 지시로 작동해 (v3 전용).
> ⚠️ 만약 태그를 **소리 내어 읽어버리면** 그 태그만 지우고 그 줄 재생성. `[radio static]` `[engine starts]`는
> 공식 목록에 없는 실험 태그라 실패 확률 있음 — 안 되면 빼고 뽑아줘 (그 소리는 내가 SFX로 얹음).

`intro1` →
```
[sighs] 어디서부터 얘기해야 하나. …부모님 얼굴은 기억이 안 난다. 진짜로. 기억나는 건 할아버지 봉고차 조수석. 그게 전부다. 할아버지는 정비사였다. 고장난 세상 여기저기를 그 차로 돌면서 나를 키웠고. [chuckles] 입버릇이 하나 있었는데. "잘 봐라. 차는 고치면 다시 달린다. 사람도 그렇다." …그땐 뭔 소린가 했지.
```
`intro2` → *(Sang 개고 v2 — "어딘가엔 아는 사람이 있겠지")*
```
[clears throat] 3년 전 봄에, 천리안이 깨어났다. 그날 무슨 일이 있었는지는… 아는 사람이 없다. 뭐, 어딘가엔 아는 사람이 있겠지. 근데 최소한 내가 여태 마주친 사람 중엔 없었다. 그게 그날 대체 무슨 짓을 한 건지. [exhales] 내가 아는 거라곤… 그날부터 도시 불이 하나씩 꺼졌다는 거. 할아버지랑 나는 미친 듯이 도망쳐서 부산 끝 부두까지 밀려났고, 거기서 고물 봉고차 한 대를 주워서 같이 고치기 시작했다는 거. [chuckles] 할아버지는 그 차에 '달구지'라는, 진짜 촌스러운 이름을 붙였다.
```
`intro3` →
```
지난겨울에… [sighs] 할아버지가 먼저 갔다. 조용한 양반이라, 가는 것도 조용했다. 남긴 건 딱 두 개야. 달구지 열쇠 하나, 손때 묻은 정비 수첩 하나. [swallows] 수첩 첫 장에… 이건 아직도 외운다. "달구지를 완성해라. 그리고 어디든… 끝까지 가라. 길에서 배운 건, 길에서 갚는 거다." [exhales] …완성이 뭔지는 안 적어놨더라. 그 양반답게.
```
`intro4` →
```
그러다 어느 날 밤이었다. 3년 내내 죽어 있던 라디오가… 갑자기 지직거리는 거야. [radio static] [whispers] 서울. 남산. 코어는 아직 가동 중. 끝내려면, 이리로. [exhales] …딱 그 말만 하고 다시 죽었다. 누가 보낸 건지 모른다. 함정일 수도 있고. 근데 있잖아, 그날 밤에 잠이 안 오더라. 할아버지였으면… 고민도 안 했을 거다. 시동부터 걸었지.
```
`intro5` →
```
그래서 지금 여기다. 연료 채웠고, 물이랑 식량 실었고. 조수석엔 수첩 올려놨다. …할아버지 자리니까. [exhales] 서울까지 400km. 뭐가 기다리는지는 모르겠다. 근데 가보면 알겠지. 가서, 끝내자. 할아버지 몫까지. [engine starts]
```

**연출 프레임 (Sang 확정)**: 인트로 = **유저한테 얘기를 들려주는 것.** "어디서부터 얘기해야 하나"로 열고 "그래서 지금 여기다"로 현재에 도착하는 구조. 화자가 자기 한계를 인정하고("어딘가엔 아는 사람이 있겠지"), 청자에게 직접 말 걸고("근데 있잖아"), 여운으로 닫는다("그땐 뭔 소린가 했지" / "그 양반답게"). 태그는 감정의 관절에만.

**🎬 타인 목소리 삽입 (스플라이스 계획)**: intro3의 수첩 유언과 intro4의 라디오 신호는 **진짜 그 목소리로 교체**함 (영화식 회상). 주인공 버전은 인용구 포함 통째로 뽑고, 아래 2개만 추가로 뽑아주면 잘라 붙이는 건 내 몫:

`gp_notebook` → **할아버지 보이스**로
```
달구지를 완성해라. 그리고 어디든… 끝까지 가라. 길에서 배운 건, 길에서 갚는 거다.
```
`radio_signal` → **무전 화자**(radio_comms와 같은 목소리)로
```
서울. 남산. 코어는 아직 가동 중. 끝내려면, 이리로.
```
(내 후반 작업: 주인공 트랙에서 인용 구간 절단 → 할아버지=카세트 질감·신호=무전 필터 입혀 삽입 → 볼륨 매칭·50ms 크로스페이드. 어색하면 주인공 단독본으로 롤백)

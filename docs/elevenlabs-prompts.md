...
## 화자 1. 인트로 앙상블 — 12장·화자별 턴

> [!important]
> 예전 `intro1`~`intro5` 장문 대본은 폐기했다. 그 대본은 현재의 12장 빌드업, 부모의 인간 확인층, 6,412명 이송, 주인공의 자발적 출발 이유와 맞지 않는다. 아래 음색은 유지하되 실제 생성 문장은 `src/03-data.js`의 `introBeats`를 한 턴씩 그대로 사용한다.
...
- 어린 나: 자연스러운 여덟 살 아이. 귀엽게 꾸미지 않고 질문의 속도를 살린다.

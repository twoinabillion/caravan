...
`intro1` →
```
[sighs] 부모님 얼굴은… 기억이 안 난다. 기억나는 건, 할아버지 봉고차 조수석. 그게 전부다. [chuckles] 할아버지는 정비사였다. 고장난 세상 여기저기를 그 차로 돌면서, 나를 키웠다. 맨날 그랬지. "잘 봐라. 차는 고치면 다시 달린다. 사람도 그렇다."
```
...
`intro3` →
```
지난겨울에… [sighs] 할아버지가 먼저 갔다. 남긴 건 두 개. 달구지 열쇠, 그리고 손때 묻은 정비 수첩. [swallows] 수첩 첫 장에 이렇게 적혀 있다. "달구지를 완성해라. 그리고 어디든… 끝까지 가라. 길에서 배운 건, 길에서 갚는 거다."
```
`intro4` →
```
그러다 어느 날 밤에, 죽어 있던 라디오가 지직거렸다. [radio static] [whispers] 서울. 남산. 코어는 아직 가동 중. 끝내려면, 이리로. [exhales] 누가 보낸 건지 모른다. 함정일 수도 있다. 근데… 할아버지였으면 시동부터 걸었겠지.
```
`intro5` →
```
연료 채웠고, 물이랑 식량 실었다. 조수석엔 수첩. …할아버지 자리다. [exhales] 서울까지 400km. 가서, 끝내자. 할아버지 몫까지. [engine starts]
```

**연기 포인트**: intro1·2의 [chuckles]=할아버지 회상에서만 살짝 웃음 / intro3 [swallows]=유언 읽기 직전 목 넘김 / intro4 [whispers]=라디오 신호를 옮겨 말하는 낮은 목소리 — 글 읽기가 아니라 **밤에 친구한테 얘기해주는 사람**이 되는 장치들.

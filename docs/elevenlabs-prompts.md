...
```
`intro2` → *(143년의 반복 정리 정본)*
```
[clears throat] 천리안이 깨어난 건 백사십삼 년 전이라고 했다. 그 뒤 세상이 한꺼번에 끝난 건 아니었다. 학교도, 시장도, 군부대도 다시 돌아갔다. 하지만 안전의 기준이 바뀔 때마다… 정리는 세대마다, 구역마다 돌아왔다. [exhales] 사람들은 서울에서 남쪽으로 밀려났다. 왜 쫓겨났는지는 아무도 모른다. 할아버지도 자기 부모에게서 같은 길 이야기를 들었다고 했다. 그러다 우리 차례가 왔고, 우리는 부산 끝 부두까지 흘러갔다.
```
`intro3` →
```
...
`intro4` →
```
그러다 어느 날 밤이었다. 죽어 있던 라디오가… 갑자기 지직거리는 거야. [radio static] [whispers] 서울. 남산. 코어는 아직 가동 중. 끝내려면, 이리로. [exhales] …딱 그 말만 하고 다시 죽었다. 누가 보낸 건지 모른다. 함정일 수도 있고. 근데 있잖아, 그날 밤에 잠이 안 오더라. 할아버지였으면… 고민도 안 했을 거다. 시동부터 걸었지.
```
`intro5` →
```

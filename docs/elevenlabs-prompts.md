...
## 화자 1. 인트로 앙상블 — 17장·화자별 턴

> [!important]
> 예전 `intro1.mp3`~`intro5.mp3` 장문 대본은 폐기했다. 그 대본은 현재의 17장 빌드업, 도윤 가족의 직접 피해, 부산 이의 제기 실패, 부모의 인간 확인층과 주인공의 자발적 출발 이유에 맞지 않는다. 아래 음색은 유지하되 실제 생성 문장은 `src/03-data.js`의 `introBeats`를 한 턴씩 그대로 사용한다.

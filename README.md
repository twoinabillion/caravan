...
| 출발 이유 | 엄마의 철제 상자에서 현재 이송 명령과 같은 규격의 남산 회로도, 달구지 계기판 속 검증 모듈을 발견한다. **첫 이송까지 스무 날** 안에 뜯긴 분리 절차·발신 기록·당사자 증언을 모아 이번 명령을 멈추는 것이 현재의 목표다 |
...
| UI·접근성 | 동료 과제 중에도 일반 의뢰와 마감이 사라지지 않는 이중 미션 표시 · 상태창에서 출발 이유와 남은 준비를 상시 확인 · 인트로 자동 진행 ON/OFF와 재방문 건너뛰기 · 기기별 큰 글자·움직임 줄임 설정 · 움직임 줄임 시 캔버스 갱신 부담 완화 · 키보드 모달 포커스 순환 · 탭 선택 상태와 장면별 스크린리더 안내 · 모바일 단일 HTML 35MB 상한 자동 검사 |

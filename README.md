...
| 영역 | 내용 |
|---|---|
| 이벤트 | **836종**(대화 195종 포함) · 잡담 243 · 티키타카 45 |
| 지도 | 노드 58곳 전부 WGS84 실측 좌표 · 도로 77 · 오프라인 여정도 + 선택형 V-World 2D 실측 지도 |
...
| 밴 | 용달 적재함에 얹은 기본 생활칸 · 업그레이드 28종 트리(전부 코드로 외형 조합+실효과) · 동료 좌석 2→3→4→5→6 단계 증설 |
| 전투 | 쇠파이프·석궁·화염병·탄약을 소비하는 이벤트 선택형 전투 |
| 시네마틱 | **장면 이미지 59종**. 12장 그림책 인트로·도시 도착·부모 서사·서울 결말을 고유 컷으로 연결하고 나머지 836개 이벤트도 공용 컷으로 전부 시각화 |
...
불변 설정은 [docs/SERIES-BIBLE.md](docs/SERIES-BIBLE.md), 실측 지도 연결과
키 보안 원칙은 [docs/vworld-map.md](docs/vworld-map.md) 참고.
...
낡은 한 톤 용달 트럭의 적재함에 최소한의 생활칸을 얹은 **달구지**를 고치고 확장하면서 부산에서 서울 남산까지 411km를 이동한다. 처음에는 겨우 먹고 잘 수 있는 작은 집이지만, 사람을 태울 때마다 좌석·침대·부엌을 덧붙인다. 날씨와 시간대가 바뀌는 도로, 생존자들과의 선택, 동료 관계, 천리안의 추적이 하나의 여정을 만든다. 무작위 사건이 많아도 본편이 묻히지 않도록 60·100·140·220·240·300km에서 세대 서사와 부모의 수정안이 다음 도착 장면으로 보장된다.

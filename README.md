...
| 영역 | 내용 |
|---|---|
| 이벤트 | **880종**(대화 195종·동료 합류 서사 18종·동료 선제 행동 6종·김천 노선 7종·정착지 도로 여파 포함) · 잡담 255 · 티키타카 45 |
...
| 전투·현장 임무 | 북부 초계·드론·검문소의 **정찰→대응→교전/이탈 3단계**에 더해, 총을 쏠 상대가 없는 능선 구조와 자동 검문소 장터 호송도 같은 지형·압박·전술 연계 규칙으로 3단계 진행된다. 성공·부분 성공은 ‘비살상 임무’ 장부에 따로 남는다 |
| 시네마틱 | **장면 이미지 114종**. 17장 그림책 인트로, 동료별 합류 18장, 전투·도로수선단·김천 갈림길·능선 구조·장터 호송·정착지 도로 여파·도시 도착·부모 서사·서울의 증언과 해방까지 고유 컷으로 연결한다 |
...
npm run verify:quick    # 대사 자연스러움 + 888개 이벤트형 콘텐츠 참조 검사

...
> **2026-08-18 구현 상태 — 전 게임 적용 완료.** 대구 검증판을 공통 셸로 일반화해
> **7개 정착지 × 장터·정비소·사람들·현장 통로 = 28화면**에 적용했다. 모든 시설은
> `행 선택 → 하단 실행 버튼 1개` 계약을 공유하며, 도착 사진은 시설 내부에서 반복하지 않는다.
> 28개 실제 캡처와 재현 스크립트는 `audits/field-board-rollout-2026-08-18/`, 320/390/475px
> 자동 계약 검사는 `tests/test_field_board_rollout.py`에 있다. 시각 마감도 전 화면에 완료해
> 의뢰는 압정 쪽지, 나머지 항목은 금속 목록으로 재질을 분리했다. 전후 증거는
> `audits/field-board-visual-rollout-2026-08-18/`에 있다.

### 실제 구현에서 확정한 범위

- 새로 만든 래스터 자산은 `assets/ui/field-board-frame-v1.webp` 하나(672 bytes)뿐이다.
- 스캔라인·시설 아이콘·실행 바 전용 이미지는 추가하지 않았다. 기존 아이콘과 CSS 질감으로 충분해
  초기 문서의 4개 자산 계획보다 용량과 유지보수 비용을 줄였다.
- 도시별 별도 사진 28장은 만들지 않았다. 기존 `settlementWorlds[*].palette`의 7개 팔레트를
  CSS 변수로 주입해 광주·밀양·대구·무주·전주·대전·수원의 판 색을 구분한다.
- 장터 거래·의뢰, 정비·개조, 주민 대화·합류·숙박, 현장 행동의 원래 게임 상태 변화는 보존했다.
